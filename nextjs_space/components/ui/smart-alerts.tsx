...
export function SmartAlerts({
  ...
  const alerts = useMemo(() => {
    ...
    projects.forEach((project) => {
      ...
      // Budget warnings
      const spent =
        project.actualCost || project.budget * (project.progress / 100) * 1.1;
        ...
        <CardTitle className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-white/80 font-normal">
              AI-generated actionable notifications
            </p>
          </div>
          {visibleAlerts.length > 0 && (
      ...
      <CardContent className="p-6">
        {visibleAlerts.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-slate-600 dark:text-slate-400">
              No critical alerts at this time. Keep up the great work!
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {visibleAlerts.map((alert) => (
              ...
              >
                ...
                >
                  <div
                    className={`p-3 rounded-xl ${getIconStyles(alert.type)}`}
                  >
                    {getAlertIcon(alert.type)}
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <ChevronRight
                      className={`h-5 w-5 text-slate-400 transition-transform ${expandedAlert === alert.id ? "rotate-90" : ""}`}
                    />
                    <Button
                      variant="ghost"
                      size="sm"
