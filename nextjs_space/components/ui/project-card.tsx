...
export function ProjectCard({ project, index = 0 }: ProjectCardProps) {
  ...
  const metrics = useMemo(() => {
    ...
    if (startDate && endDate) {
      ...
      if (scheduleProgress > taskCompletion + 10) {
        scheduleHealth = Math.max(
          0,
          100 - (scheduleProgress - taskCompletion) * 2,
        );
      }
    }
    ...
    const budgetHealth =
      budget > 0
        ? Math.min(
            100,
            Math.round(((budget - estimatedSpend) / budget) * 100) + 50,
          )
        : 100;
    ...
    let healthLevel: "critical" | "at-risk" | "on-track" | "excellent" =
      "on-track";
  ...
  const getHealthIcon = (level: string) => {
    switch (level) {
      ...
      case "on-track":
        return <Activity className="h-4 w-4" style={{ color: "#3b82f6" }} />;
      case "at-risk":
  ...
  return (
    ...
    >
      <Link href={`/projects/${project?.id ?? ""}`}>
        <Card
          className="h-full hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer overflow-hidden"
          style={{ backgroundColor: "#ffffff" }}
        >
          {/* Health indicator strip */}
          <div
            className="h-1.5"
            style={{ backgroundColor: getHealthColor(metrics.healthLevel) }}
          />
          ...
          <CardContent className="p-5">
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1 min-w-0">
                <h3
                  className="font-semibold text-base mb-0.5 line-clamp-1"
                  style={{ color: "#0f172a" }}
                >
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <span
                  ...
                  style={{ color: getHealthColor(metrics.healthLevel) }}
                >
                  {metrics.healthScore}%
                </span>
              </div>
            ...
            >
              ...
              {project?.endDate && (
                <div className="flex items-center gap-1">
                  ...
                  <span
                    style={{ color: metrics.isOverdue ? "#ef4444" : "#64748b" }}
                  >
                    {metrics.isOverdue
                      ? `${metrics.daysRemaining}d overdue`
            ...
            {(metrics.overdueTasks > 0 || metrics.criticalTasks > 0) && (
              <div className="flex flex-wrap gap-2 mb-3">
                {metrics.overdueTasks > 0 && (
                  ...
                  >
                    <AlertTriangle className="h-3 w-3" /> {metrics.overdueTasks}{" "}
                    overdue
                  </span>
                )}
            ...
            {project?.budget && (
              <div
                className="flex items-center justify-between text-xs py-2 border-t"
