...
export function ResourceIntelligence({
  ...
  tasks = [],
  compact = false,
}: ResourceIntelligenceProps) {
  const [showDetails, setShowDetails] = useState(false);

  const analysis = useMemo(() => {
    const now = new Date();
    const allTasks =
      tasks.length > 0 ? tasks : projects.flatMap((p) => p.tasks || []);
    ...
    const workloadAnalysis: WorkloadAnalysis[] = teamMembers.map((member) => {
      ...
      const highPriorityImpact =
        memberTasks.filter(
          (t) => t.priority === "HIGH" || t.priority === "CRITICAL",
        ).length * 10;
      const workloadScore = Math.min(
        100,
        baseWorkload + overdueImpact + highPriorityImpact,
      );
  ...
  const summary = useMemo(() => {
    ...
    const optimal = analysis.filter(
      (a) => a.workloadScore >= 30 && a.workloadScore <= 75,
    ).length;
    const underutilized = analysis.filter((a) => a.workloadScore < 30).length;
    const avgEfficiency =
      analysis.length > 0
        ? Math.round(
            analysis.reduce((sum, a) => sum + a.efficiency, 0) /
              analysis.length,
          )
        : 0;
            ...
            {analysis.slice(0, 3).map((member) => {
              ...
              return (
                ...
                >
                  ...
                  <Badge className={`text-xs ${workloadInfo.color}`}>
                    {workloadInfo.text}
                  </Badge>
                </div>
              );
        ...
        <CardTitle className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-white/80 font-normal">
              Team workload & capacity optimization
            </p>
          </div>
          <Badge className="ml-auto bg-white/20 text-white border-0">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-700 dark:text-slate-300">
                  {summary.overloaded} team member
                  {summary.overloaded > 1 ? "s are" : " is"} overloaded while{" "}
                  {summary.underutilized} ha
                  {summary.underutilized !== 1 ? "ve" : "s"} available capacity.
                  Consider redistributing {Math.min(5, summary.totalOverdue)}{" "}
                  task{summary.totalOverdue !== 1 ? "s" : ""} to balance
                  workload.
                </p>
              </div>
            </div>
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-right">
                    ...
                    <Progress
                      value={member.workloadScore}
                      className={`h-2 w-24 ${getWorkloadColor(member.workloadScore)}`}
                    />
                  </div>
                </div>
