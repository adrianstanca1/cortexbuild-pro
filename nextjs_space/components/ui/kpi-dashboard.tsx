...
const cardVariants = {
  ...
  alert: "border-red-200 dark:border-red-900/50 bg-red-50/50 dark:bg-red-900/10",
  success: "border-green-200 dark:border-green-900/50 bg-green-50/50 dark:bg-green-900/10"
};

export function KPICard({ title, icon, metrics, link, variant = "default", className }: KPICardProps) {
  return (
    <div className={cn(
...
export function QuickAction({ icon, label, href, variant = "secondary" }: QuickActionProps) {
  const variants = {
    primary: "bg-gradient-to-r from-primary to-purple-600 text-white shadow-lg shadow-primary/25 hover:shadow-xl hover:shadow-primary/30",
    secondary: "bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 hover:border-primary/50 dark:hover:border-primary/50"
  };
...
export function AlertBanner({ type, title, message, action, className }: AlertBannerProps) {
  ...
  return (
    <div className={cn("flex items-start gap-4 p-4 rounded-xl border", style.bg, className)}>
      ...
      {action && (
        <Link 
          href={action.href}
          className={cn("text-sm font-medium hover:underline flex-shrink-0", style.text)}
        >
          {action.label}
        </Link>
      )}
