...
function calculateProjectMetrics(project: ProjectData): ProjectMetrics {
  ...
  const avgRfiAge = rfis.filter(r => r.status === 'OPEN').reduce((acc, r) => {
  ...
  const criticalIncidents = safetyIncidents.filter(s => s.severity === 'CRITICAL' || s.severity === 'HIGH').length;
  ...
  const plannedProgress = milestones.length > 0 
    ? milestones.filter(m => new Date(m.targetDate) <= new Date()).length / milestones.length * 100
    : taskProgress;
  ...
  if (cpi >= 0.95 && cpi <= 1.05 && totalActual > 0) {
    insights.push({
      ...
      description: 'Project spending is within acceptable variance of estimates.',
      metric: `CPI: ${cpi.toFixed(2)}`
    });
  }

  if (safetyIncidents.length === 0 || (openIncidents === 0 && criticalIncidents === 0)) {
    insights.push({
      id: 'safety-good',
      type: 'success',
      category: 'safety',
      title: 'Strong Safety Record',
      description: 'No open safety incidents. Maintain current safety protocols.',
      metric: '0 incidents'
    });
  }
  ...
  if (project.endDate) {
    ...
    if (spi < 0.9) {
      const delay = Math.round((1 / spi - 1) * ((endDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24)));
...
export function ProjectIntelligence({ project, compact = false }: ProjectIntelligenceProps) {
    ...
    return (
      <Card className="border-slate-200 dark:border-slate-700">
        <CardContent className="py-10 flex flex-col items-center justify-center">
          ...
          <p className="text-sm text-muted-foreground mt-4">Analyzing project data...</p>
        </CardContent>
      </Card>
    );
  }
  ...
  const getHealthBg = (score: number) => {
    if (score >= 85) return 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-700/50';
    if (score >= 70) return 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-700/50';
    if (score >= 50) return 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-700/50';
  ...
  const displayedInsights = expanded ? metrics.insights : metrics.insights.slice(0, 3);

  if (compact) {
    return (
      <div className={`flex items-center gap-4 p-4 rounded-xl border ${getHealthBg(metrics.healthScore)}`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600 dark:text-slate-300 font-medium">Health Score</p>
          </div>
        </div>
        <div className="flex-1 flex items-center gap-4">
          ...
          <p className="text-xs text-slate-600 dark:text-slate-300 hidden sm:block font-medium">{metrics.completionPrediction}</p>
        </div>
    ...
    <Card className="border-slate-200 dark:border-slate-700 overflow-hidden bg-white dark:bg-slate-800">
      <CardHeader className="pb-3 border-b border-slate-100 dark:border-slate-700 bg-gradient-to-r from-slate-50 to-slate-100/50 dark:from-slate-800/50 dark:to-slate-700/30">
        <CardTitle className="text-sm font-semibold flex items-center gap-2 text-slate-800 dark:text-slate-100">
          ...
          <Badge variant="outline" className="ml-auto text-xs font-medium bg-white dark:bg-slate-700 border-primary/30 text-primary">
      ...
      <CardContent className="p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          {/* Health Score */}
          <div className={`text-center p-3 rounded-xl border ${getHealthBg(metrics.healthScore)}`}>
            <div className="relative inline-flex items-center justify-center">
              ...
              <span className={`absolute text-lg font-bold ${getHealthColor(metrics.healthScore)}`}>
                {metrics.healthScore}
              </span>
            </div>
            <div className="text-xs text-slate-600 dark:text-slate-300 mt-1 font-semibold">Health</div>
          </div>
          ...
          <div className="text-center p-3 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700/50">
            <div className="flex items-center justify-center gap-1">
              ...
              <span className="text-xl font-bold text-blue-700 dark:text-blue-300">{metrics.schedulePerformance.toFixed(2)}</span>
            </div>
            <div className="text-xs text-slate-600 dark:text-slate-300 mt-1 font-medium">Schedule (SPI)</div>
          </div>
          ...
          <div className="text-center p-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700/50">
            <div className="flex items-center justify-center gap-1">
              ...
              <span className="text-xl font-bold text-emerald-700 dark:text-emerald-300">{metrics.costPerformance.toFixed(2)}</span>
            </div>
            <div className="text-xs text-slate-600 dark:text-slate-300 mt-1 font-medium">Cost (CPI)</div>
          </div>
          ...
          <div className="text-center p-3 rounded-xl bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700/50">
            <Badge className={`${getRiskBadge(metrics.riskLevel)} text-xs font-medium`}>
              {metrics.riskLevel.toUpperCase()}
            </Badge>
            <div className="text-xs text-slate-600 dark:text-slate-300 mt-2 font-medium">Risk Level</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 mb-4 p-3 bg-slate-100 dark:bg-slate-700/50 rounded-lg">
          ...
          <span className="text-sm text-slate-700 dark:text-slate-200">Forecast: <strong className="text-slate-800 dark:text-slate-100">{metrics.completionPrediction}</strong></span>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-slate-600 dark:text-slate-300 font-medium">{metrics.insights.length} generated</span>
          </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-semibold text-slate-800 dark:text-slate-100">{insight.title}</span>
                    {insight.metric && (
                      <Badge variant="outline" className="text-xs font-medium text-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-500">{insight.metric}</Badge>
                    )}
                  </div>
                  <p className="text-xs text-slate-600 dark:text-slate-300 mt-1 font-medium">{insight.description}</p>
                  {insight.action && (
