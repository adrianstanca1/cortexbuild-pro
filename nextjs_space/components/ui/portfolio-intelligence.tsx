...
function calculatePortfolioMetrics(
  ...
  const overallHealth =
    projects.length > 0 ? Math.round(totalHealthScore / projects.length) : 100;
  ...
  const safetyScore =
    ...
    constructionMetrics?.safetyIncidentsThisMonth < 2
      ? 95
      : constructionMetrics?.criticalIncidents > 0
        ? 50
        : 75;
  ...
  const budgetHealth =
    constructionMetrics?.pendingChangeOrders === 0
      ? 95
      : constructionMetrics?.pendingChangeOrders < 3
        ? 80
        : 65;
...
export function PortfolioIntelligence({
  ...
  if (compact) {
    return (
      <div
        className={`flex items-center gap-4 p-4 rounded-xl border ${getHealthBg(metrics.overallHealthScore)}`}
      >
        <div className="flex items-center gap-3">
          <div
            className={`p-2 rounded-lg bg-gradient-to-br ${getHealthGradient(metrics.overallHealthScore)}`}
          >
            <Brain className="h-5 w-5 text-white" />
          </div>
          <div>
            <span
              className={`text-2xl font-bold ${getHealthColor(metrics.overallHealthScore)}`}
            >
    ...
    <Card className="border-slate-200 dark:border-slate-700 overflow-hidden bg-white dark:bg-slate-800">
      <CardHeader className="border-b border-slate-100 dark:border-slate-700 bg-gradient-to-r from-slate-50 to-slate-100/50 dark:from-slate-800/50 dark:to-slate-700/30 pb-4">
        <div className="flex items-center justify-between">
          ...
          <Badge
            variant="outline"
            className="text-xs bg-white dark:bg-slate-700 border-primary/30 text-primary font-medium"
          >
      ...
      <CardContent className="p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div
            className={`col-span-2 md:col-span-1 p-4 rounded-xl border ${getHealthBg(metrics.overallHealthScore)} text-center`}
          >
            <div className="relative inline-flex items-center justify-center">
              ...
              <span
                className={`absolute text-2xl font-bold ${getHealthColor(metrics.overallHealthScore)}`}
              >
                {metrics.overallHealthScore}
              </span>
            </div>
          ...
          <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700/50 text-center">
            <div className="flex items-center justify-center gap-2 mb-1">
              ...
              <span
                className={`text-xl font-bold ${metrics.scheduleHealth >= 70 ? "text-blue-700 dark:text-blue-300" : "text-amber-700 dark:text-amber-300"}`}
              >
            ...
            <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1 font-medium">
              {metrics.totalOverdueTasks} overdue tasks
            </p>
          </div>
          ...
          <div className="p-4 rounded-xl bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700/50 text-center">
            <div className="flex items-center justify-center gap-2 mb-1">
              ...
              <span
                className={`text-xl font-bold ${metrics.safetyScore >= 70 ? "text-purple-700 dark:text-purple-300" : "text-red-700 dark:text-red-300"}`}
              >
                {metrics.safetyScore}%
              </span>
            </div>
            ...
            <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1 font-medium">
              {metrics.totalCriticalIssues} critical issues
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {(expanded ? metrics.insights : metrics.insights.slice(0, 3)).map(
              (insight) => (
                <div
                  key={insight.id}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {insight.metric && (
                          <Badge
                            variant="outline"
                            className="text-xs shrink-0 font-medium text-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-500"
                          >
                            {insight.metric}
                          </Badge>
                        )}
