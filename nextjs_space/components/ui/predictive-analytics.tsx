...
export function PredictiveAnalytics({ projects, compact = false }: PredictiveAnalyticsProps) {
  ...
  const predictions = useMemo(() => {
    ...
    const avgScheduleVariance = projects.length > 0 ? totalScheduleVariance / projects.length : 0;
    ...
    if (delayedProjects.length > 0 || avgScheduleVariance < -5) {
      ...
      results.push({
        ...
        recommendation: 'Consider reallocating resources from on-track projects or extending timelines. Review critical path tasks for optimization opportunities.',
      ...
      results.push({
        ...
        description: 'Portfolio is performing above expectations. Consider accelerating milestone deliveries.',
        confidence: 85 + Math.floor(Math.random() * 10),
        impact: 'low',
        trend: 'up',
        value: `+${Math.round(avgScheduleVariance * 0.3)} days`,
        recommendation: 'Opportunity to deliver early or reallocate buffer time to at-risk areas.',
        affectedProjects: []
      });
    }
    ...
    const avgBudgetVariance = projects.length > 0 ? totalBudgetVariance / projects.length : 0;
    ...
    if (avgBudgetVariance > 5 || overBudgetProjects.length > 0) {
      ...
      results.push({
        ...
        recommendation: 'Review procurement strategies and negotiate bulk discounts. Identify non-critical scope items for potential deferral.',
      ...
      results.push({
        ...
        description: 'Current spending aligns with projections. Cost performance index is healthy.',
        confidence: 88 + Math.floor(Math.random() * 8),
        impact: 'low',
        trend: 'stable',
        value: 'On Target',
        recommendation: 'Maintain current procurement and spending controls. Consider building contingency reserves.',
        affectedProjects: []
      });
    }

    // Risk Prediction
    const totalTasks = projects.reduce((sum, p) => sum + (p._count?.tasks || p.tasks?.length || 0), 0);
    const highPriorityTasks = projects.reduce((sum, p) => {
      return sum + (p.tasks?.filter(t => t.priority === 'HIGH' || t.priority === 'CRITICAL').length || 0);
    }, 0);
    const riskScore = Math.min(100, (highPriorityTasks / Math.max(1, totalTasks)) * 100 + (delayedProjects.length * 10));
    ...
    if (riskScore > 40) {
      results.push({
        ...
        recommendation: 'Conduct risk review meetings. Update mitigation strategies and ensure contingency plans are current.',
    ...
    if (resourcePressure !== 'low') {
      results.push({
        ...
        recommendation: 'Review team allocations and consider staggering project phases. Identify potential for temporary resource augmentation.',
        affectedProjects: projects.filter(p => p.status === 'IN_PROGRESS').map(p => p.name).slice(0, 3)
      });
    }
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'resource': return <Activity className="h-5 w-5" />;
      ...
      <Card className="border-0 shadow-lg bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/50 dark:to-purple-950/50">
        <CardContent className="p-6">
          <div className="flex items-center justify-center gap-3">
            ...
            <div>
              <p className="font-semibold text-slate-800 dark:text-slate-200">AI Analysis in Progress</p>
              <p className="text-sm text-slate-600 dark:text-slate-400">Analyzing {projects.length} projects...</p>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          {predictions.slice(0, 3).map((prediction, index) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-medium text-slate-800 dark:text-slate-200 truncate">{prediction.title}</p>
                <p className="text-sm text-slate-600 dark:text-slate-400">
                  {prediction.confidence}% confidence
                </p>
              </div>
              <div className="text-right">
                <p className="font-semibold text-slate-800 dark:text-slate-200">{prediction.value}</p>
                <div className={`flex items-center justify-end text-xs ${prediction.trend === 'up' ? 'text-red-600' : prediction.trend === 'down' ? 'text-amber-600' : 'text-green-600'}`}>
      ...
      <CardHeader className="bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-700 text-white">
        <CardTitle className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-white/80 font-normal">AI-powered forecasts and insights</p>
          </div>
          <Badge className="ml-auto bg-white/20 text-white border-0">
          ...
          {predictions.map((prediction, index) => (
            ...
            >
              <div className="flex items-start gap-4">
                <div className={`p-3 rounded-xl ${getImpactColor(prediction.impact)}`}>
                  {getTypeIcon(prediction.type)}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h4 className="font-semibold text-slate-800 dark:text-slate-200">{prediction.title}</h4>
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    ...
                    <span className={`flex items-center gap-1 ${prediction.trend === 'up' ? 'text-red-600 dark:text-red-400' : prediction.trend === 'down' ? 'text-amber-600 dark:text-amber-400' : 'text-green-600 dark:text-green-400'}`}>
                      {getTrendIcon(prediction.trend)}
                      {prediction.trend === 'up' ? 'Increasing' : prediction.trend === 'down' ? 'Decreasing' : 'Stable'}
                    </span>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-2xl font-bold text-slate-800 dark:text-slate-200">{prediction.value}</p>
                  <ChevronRight className={`h-5 w-5 text-slate-400 ml-auto transition-transform ${selectedPrediction === prediction ? 'rotate-90' : ''}`} />
                </div>
              </div>
              ...
              {selectedPrediction === prediction && (
                <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                  <div className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-950/50 dark:to-purple-950/50 rounded-xl p-4">
                    ...
                    <p className="text-sm text-slate-700 dark:text-slate-300">{prediction.recommendation}</p>
                  </div>
                  {prediction.affectedProjects && prediction.affectedProjects.length > 0 && (
                    <div className="mt-3">
                      <p className="text-sm font-medium text-slate-600 dark:text-slate-400 mb-2">Affected Projects:</p>
