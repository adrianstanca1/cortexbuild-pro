...
const eventMessages: Partial<
  Record<RealtimeEventType, (payload: Record<string, unknown>) => string>
> = {
  // Tasks
  ...
  daily_report_updated: (p) =>
    `Daily report updated${p.projectName ? ` for ${p.projectName}` : ""}`,
  daily_report_deleted: (_p) => `Daily report deleted`,
  // Safety
  safety_incident_reported: (p) =>
    `⚠️ Safety incident reported${p.severity ? ` (${p.severity})` : ""}`,
  safety_incident_updated: (p) =>
    `Safety incident updated${p.projectName ? ` for ${p.projectName}` : ""}`,
  safety_incident_resolved: (p) =>
    `✓ Safety incident resolved${p.projectName ? ` for ${p.projectName}` : ""}`,
  ...
  change_order_approved: (p) =>
    `✓ Change order approved: ${getField(p, "title")}`,
  // Punch Lists
  ...
  // Activity
  activity_logged: (p) =>
    `${p.userName || "Someone"} ${p.action || "performed an action"}${p.entityName ? ` on ${p.entityName}` : ""}`,
};
...
export function RealtimeProvider({
  ...
  const subscribersRef = useRef<
    Map<RealtimeEventType, Set<(event: RealtimeEvent) => void>>
  >(new Map());
  ...
  return (
    <RealtimeContext.Provider
      value={{ isConnected, lastEvent, connectedClients, subscribe }}
    >
      {children}
    </RealtimeContext.Provider>
  );
