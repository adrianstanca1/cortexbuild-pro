...
export async function POST(request: NextRequest) {
  try {
    ...
    const query =
      ...
      "Analyse this construction document and provide a detailed summary, key risks, and action items.";
    ...
    if (fileType === "text/plain" || fileName.endsWith(".txt")) {
      ...
      return NextResponse.json(
        {
          error:
            "Unsupported file type. Supported: TXT, CSV, MD, PDF (text-only), images (vision model required)",
        },
        { status: 400 },
      );
    }
    ...
    if (!available) {
      return NextResponse.json(
        {
          error:
            "AI service unavailable. Ensure Ollama is running at " +
    ...
    const messages = [
      {
        ...
        content:
          "You are an AI assistant specialising in construction document analysis for CortexBuildPro, used by AS Cladding & Roofing Ltd. Analyse documents for: compliance issues, safety concerns, cost implications, schedule impacts, and actionable insights. Be thorough but concise.",
      },
      {
        role: "user" as const,
        content: `${query}\n\nDocument: ${file.name}\n\n---\n${textContent}\n---`,
