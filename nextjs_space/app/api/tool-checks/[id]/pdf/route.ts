...
function generateToolCheckPDF(check: any) {
  ...
  const getCheckItems = () => {
    ...
    if (check.properSize !== undefined) items.push({ name: 'Proper Size for Job', value: check.properSize });
    ...
    if (check.lockingMechanismOk !== undefined) items.push({ name: 'Locking Mechanism OK', value: check.lockingMechanismOk });
    if (check.weightCapacityVisible !== undefined) items.push({ name: 'Weight Capacity Visible', value: check.weightCapacityVisible });
    if (check.noLooseHardware !== undefined) items.push({ name: 'No Loose Hardware', value: check.noLooseHardware });
    ...
    if (check.bracingIntact !== undefined) items.push({ name: 'Cross-Bracing Intact', value: check.bracingIntact });
    if (check.basePlatesLevel !== undefined) items.push({ name: 'Base Plates Level', value: check.basePlatesLevel });
    if (check.accessLaddersSecure !== undefined) items.push({ name: 'Access Ladders Secure', value: check.accessLaddersSecure });
