...
export async function GET(request: NextRequest) {
  try {
    ...
    const categoryBreakdown: Record<
      string,
      { estimated: number; actual: number; committed: number; count: number }
    > = {};
    ...
    const variancePercent =
      totalEstimated > 0 ? ((variance / totalEstimated) * 100).toFixed(2) : "0";

    // Monthly burn rate (last 3 months)
    const threeMonthsAgo = new Date();
    threeMonthsAgo.setMonth(threeMonthsAgo.getMonth() - 3);

    const recentCosts = costItems.filter(
      (item) => item.createdAt && new Date(item.createdAt) >= threeMonthsAgo,
    );
    const recentTotal = recentCosts.reduce(
    ...
    const projectedAtCompletion =
      totalActual + (totalCommitted - totalActual) * (1 + contingencyPercent);
    ...
    return NextResponse.json({
      ...
      categoryBreakdown: Object.entries(categoryBreakdown).map(
        ([category, data]) => ({
          ...
          percentOfTotal:
            totalEstimated > 0
              ? ((data.estimated / totalEstimated) * 100).toFixed(1)
              : "0",
        }),
      ),
      monthlyTrend,
