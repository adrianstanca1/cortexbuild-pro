...
export async function GET(_request: NextRequest) {
  try {
    ...
    const [
      ...
      prisma.task.count({
        where: { project: { organizationId: orgId }, status: "COMPLETE" },
      }),
      prisma.task.count({
        where: {
    ...
    const taskCompletionRate =
      totalTasksNum > 0
        ? Math.round((completedTasksNum / totalTasksNum) * 100)
        : 0;
    const projectCompletionRate =
      totalProjectsNum > 0
        ? Math.round((completedProjectsNum / totalProjectsNum) * 100)
        : 0;
    ...
    return NextResponse.json(
      bigintSafe({
        ...
        modules: {
          ...
          submittals: {
            total: Number(totalSubmittals),
            pending: Number(pendingSubmittals),
          },
          changeOrders: { total: Number(totalChangeOrders) },
          safety: {
            total: Number(totalSafetyIncidents),
        ...
        activity: {
          ...
          weeklyTrend: weeklyActivity.map((w) => ({
            date: w.date,
