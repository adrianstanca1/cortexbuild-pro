    ...
    const analytics = {
      ...
      documentation: {
        ...
        submittals: {
          ...
          pending:
            submittalStats.find((s) => s.status === "SUBMITTED")?._count || 0,
          byStatus: submittalStats.reduce(
            (acc, s) => {
      ...
      quality: {
        ...
        inspections: {
          ...
          passed: inspectionStats
            .filter((i) => i.status === "PASSED")
            .reduce((acc, i) => acc + i._count, 0),
          failed: inspectionStats
            .filter((i) => i.status === "FAILED")
            .reduce((acc, i) => acc + i._count, 0),
          byStatus: inspectionStats.reduce(
            (acc, i) => {
