...
export async function GET(
  ...
  try {
    ...
    if (!source || source === 'daily_report') {
      ...
      // Batch fetch all S3 URLs in parallel
    ...
    if (!source || source === 'safety_incident') {
      ...
      // Flatten all photos and batch fetch S3 URLs
      const allIncidentPhotos = safetyIncidents.flatMap(incident => 
        incident.photos.map(photo => ({ incident, photo }))
      );
      ...
      allIncidentPhotos.forEach(({ incident, photo }, index) => {
