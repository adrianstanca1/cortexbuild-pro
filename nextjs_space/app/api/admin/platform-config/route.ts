...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (
      ...
      updates.maintenanceMode !== previousConfig.maintenanceMode
    ) {
      broadcastToAll({
        ...
        data: {
          ...
          message:
            updates.maintenanceMessage || platformConfig.maintenanceMessage,
          timestamp: new Date().toISOString(),
        },
      });
    }
...
function validateConfig(config: any): { valid: boolean; errors: string[] } {
  ...
  if (
    config.maxProjectsPerOrg &&
    (config.maxProjectsPerOrg < 1 || config.maxProjectsPerOrg > 10000)
  ) {
    errors.push("maxProjectsPerOrg must be between 1 and 10000");
  }
  if (
    config.maxUsersPerOrg &&
    (config.maxUsersPerOrg < 1 || config.maxUsersPerOrg > 10000)
  ) {
    errors.push("maxUsersPerOrg must be between 1 and 10000");
  }
  if (
    config.security?.passwordMinLength &&
    config.security.passwordMinLength < 6
  ) {
