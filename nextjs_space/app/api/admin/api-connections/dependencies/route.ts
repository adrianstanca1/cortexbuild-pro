  ...
  try {
    ...
    modules.forEach(modId => {
      const relatedServices = instances.filter(inst =>
        inst.definition.dependencies.some(dep => dep.moduleId === modId || dep.moduleId === "all")
      );
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        fullyOperational: Object.values(dependencyMap).filter(
          m => m.services.filter(s => s.isRequired).every(s => s.status === "ACTIVE")
        ).length,
        degraded: Object.values(dependencyMap).filter(
          m => m.services.filter(s => s.isRequired).some(s => s.status !== "ACTIVE")
        ).length
      }
    });
  } catch (error) {
