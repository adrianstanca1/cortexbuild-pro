...
export async function GET(
  ...
  try {
    ...
    const changeOrder = await prisma.changeOrder.findUnique({
      ...
      include: {
        project: {
          select: { id: true, name: true, organizationId: true, budget: true },
        },
...
export async function PATCH(
  ...
  try {
    ...
    const { status, title, description, reason, costChange, scheduleChange } =
      body;
    ...
    if (scheduleChange !== undefined)
      updateData.scheduleChange = scheduleChange;
    ...
    if (status !== undefined) {
      ...
      if (status === "APPROVED") {
        ...
        await prisma.project.update({
          ...
          data: {
            budget:
              (existingCO.originalBudget || 0) + (existingCO.costChange || 0),
          },
        });
      }
      if (status === "EXECUTED") {
...
export async function DELETE(
  ...
  try {
    ...
    if (!["DRAFT", "REJECTED"].includes(changeOrder.status)) {
      return NextResponse.json(
        { error: "Can only delete draft or rejected change orders" },
        { status: 400 },
      );
    }
