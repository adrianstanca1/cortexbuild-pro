...
export async function PATCH(
  ...
  try {
    ...
    if (correctiveAction !== undefined)
      updateData.correctiveAction = correctiveAction || null;
    if (preventiveAction !== undefined)
      updateData.preventiveAction = preventiveAction || null;
    if (injuryDescription !== undefined)
      updateData.injuryDescription = injuryDescription || null;
...
export async function DELETE(
  ...
  try {
    ...
    if (
      !session?.user ||
      !["ADMIN", "PROJECT_MANAGER", "SUPER_ADMIN", "COMPANY_OWNER"].includes(
        session.user.role,
      )
    ) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 403 });
