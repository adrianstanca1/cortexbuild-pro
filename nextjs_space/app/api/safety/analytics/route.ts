  ...
  try {
    ...
    const summary = {
      toolboxTalks: {
        ...
        totalAttendees: toolboxTalks.reduce(
          (sum, t) => sum + (t._count?.attendees || 0),
          0,
        ),
      ...
      mewpChecks: (() => {
        ...
        return {
          ...
          passRate:
            mewpChecks.length > 0
              ? Math.round((stats.passed / mewpChecks.length) * 100)
              : 0,
        };
      })(),
      toolChecks: (() => {
        const stats = toolChecks.reduce(
          (acc, c) => {
            if (c.overallStatus === "PASS") acc.passed++;
            else if (c.overallStatus === "FAIL") acc.failed++;
            else if (c.overallStatus === "NEEDS_ATTENTION")
        ...
        return {
          ...
          passRate:
            toolChecks.length > 0
              ? Math.round((stats.passed / toolChecks.length) * 100)
              : 0,
          byType: stats.byType,
        };
      })(),
      safetyIncidents: (() => {
        const stats = safetyIncidents.reduce(
          (acc, i) => {
      ...
      inspections: (() => {
        const stats = inspections.reduce(
          (acc, i) => {
            if (i.status === "PASSED") {
              acc.passed++;
              acc.completed++;
              ...
              acc.failed++;
              acc.completed++;
            } else if (i.status === "SCHEDULED" || i.status === "IN_PROGRESS")
              acc.pending++;
            return acc;
          },
    ...
    const toolboxScore =
      summary.toolboxTalks.total > 0
        ? (summary.toolboxTalks.completed / summary.toolboxTalks.total) * 100
        : 100;
    ...
    const projectBreakdown = projects
      .map((p) => {
        ...
        const toolboxCompleted = pToolbox.reduce(
          (sum, t) => sum + (t.status === "COMPLETED" ? 1 : 0),
          0,
        );
        const mewpPassed = pMewp.reduce(
          (sum, c) => sum + (c.overallStatus === "PASS" ? 1 : 0),
          0,
        );
        const toolPassed = pTool.reduce(
          (sum, c) => sum + (c.overallStatus === "PASS" ? 1 : 0),
          0,
        );
        const criticalIncidents = pIncidents.reduce(
          (sum, i) =>
            sum + (i.severity === "CRITICAL" || i.severity === "HIGH" ? 1 : 0),
          0,
        );
