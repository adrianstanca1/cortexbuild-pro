  ...
  try {
    ...
    switch (reportType) {
      case "executive_summary": {
        const [projects, tasks, rfis, submittals, changeOrders, incidents] =
          await Promise.all([
        ...
        reportData = {
          ...
          submittalSummary: submittals.reduce(
            (acc, s) => {
          ...
          changeOrderSummary: {
            ...
            total: changeOrders._sum?.costChange || 0,
          },
      ...
      case "project_status": {
        if (!projectId) {
          return NextResponse.json(
            { error: "Project ID required for status report" },
            { status: 400 },
          );
        }
      ...
      case "financial": {
        ...
        reportData = {
          ...
          summary: {
            ...
            totalAmount: changeOrders.reduce(
              (sum, co) => sum + (co.costChange || 0),
              0,
            ),
            byStatus: coByStatus.reduce(
              (acc, s) => {
                acc[s.status] = {
                  count: s._count,
                  amount: s._sum?.costChange || 0,
                };
                return acc;
              },
      ...
      case "daily_log": {
        ...
        const avgWorkers =
          reports.length > 0
            ? Math.round(
                reports.reduce((sum, r) => sum + (r.manpowerCount || 0), 0) /
                  reports.length,
              )
            : 0;
        ...
        reportData = {
          ...
          summary: {
            ...
            totalWorkerDays: reports.reduce(
              (sum, r) => sum + (r.manpowerCount || 0),
              0,
            ),
          },
        };
    ...
    if (format === "csv" && reportType === "daily_log") {
      const reports = reportData.reports as {
        reportDate: Date;
        project: { name: string };
      ...
      const csv = [
        ...
        ...reports.map(
          (r) =>
            `${new Date(r.reportDate).toISOString().split("T")[0]},"${r.project?.name || "N/A"}",${r.manpowerCount || 0},${r.weather || "N/A"}`,
        ),
      ].join("\n");
