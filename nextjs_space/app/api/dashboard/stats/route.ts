  ...
  try {
    ...
    const stats = {
      overview: {
        ...
        activeProjects: projects.find(p => p.status === 'IN_PROGRESS')?._count || 0,
        completedProjects: projects.find(p => p.status === 'COMPLETED')?._count || 0,
        teamMembers,
      },
      tasks: {
        total: tasks.reduce((acc, t) => acc + t._count, 0),
        byStatus: tasks.filter(t => t.status).reduce((acc, t) => {
          acc[t.status] = (acc[t.status] || 0) + t._count;
          return acc;
        }, {} as Record<string, number>),
        byPriority: tasks.filter(t => t.priority).reduce((acc, t) => {
      ...
      documents: {
        ...
        submittals: {
          ...
          byStatus: submittals.reduce((acc, s) => {
      ...
      safety: {
        ...
        bySeverity: safetyIncidents.reduce((acc, s) => {
