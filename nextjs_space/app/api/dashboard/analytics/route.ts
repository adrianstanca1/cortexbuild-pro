  ...
  try {
    ...
    if (type === "resource-allocation") {
      ...
      const allocation = teamMembers.map((member) => ({
        ...
        utilizationRate: Math.min(
          100,
          ((userTimeMap[member.user.id] || 0) / 160) * 100,
        ), // Assuming 160 hours/month capacity
      }));
    ...
    if (type === "budget-summary") {
      ...
      const summary = projects.map((project) => {
        ...
        const approvedCOs = project.changeOrders.reduce(
          (sum, co) => sum + (co.costChange || 0),
          0,
        );
        const revisedBudget = originalBudget + approvedCOs;
        const actualSpend = project.costItems.reduce(
          (sum, ci) => sum + (ci.actualAmount || 0),
          0,
        );
        const estimatedSpend = project.costItems.reduce(
          (sum, ci) => sum + (ci.estimatedAmount || 0),
          0,
        );
        const claimed = project.progressClaims.reduce(
          (sum, pc) => sum + (pc.thisClaim || 0),
          0,
        );
        ...
        return {
          ...
          variancePercent:
            revisedBudget > 0
              ? ((revisedBudget - actualSpend) / revisedBudget) * 100
              : 0,
        ...
        const plannedProgress =
          totalMilestones > 0
            ? project.milestones.filter(
                (m) => new Date(m.targetDate) <= new Date(),
    ...
    const [projectCount, taskCount, rfiCount, safetyCount] = await Promise.all([
      ...
      prisma.task.count({
        where: { project: { organizationId }, status: { not: "COMPLETE" } },
      }),
      prisma.rFI.count({
