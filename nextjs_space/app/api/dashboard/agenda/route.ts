...
export async function GET(_request: NextRequest) {
  try {
    ...
    const projectsNeedingReports = allProjects.filter(
      (p) => !projectsWithReports.has(p.id),
    );
    ...
    const agendaItems = [
      ...
      ...inspections.map((i) => ({
        ...
        isOverdue: i.scheduledDate
          ? new Date(i.scheduledDate) < todayStart && i.status === "SCHEDULED"
          : false,
      })),
      // Toolbox Talks
      ...toolboxTalks.map((t) => ({
      ...
      ...rfis.map((r) => ({
        ...
        isOverdue: r.dueDate
          ? new Date(r.dueDate) < todayStart && r.status !== "CLOSED"
          : false,
      })),
      // Submittals
      ...submittals.map((s) => ({
        ...
        isOverdue: s.dueDate
          ? new Date(s.dueDate) < todayStart && s.status !== "APPROVED"
          : false,
      })),
    ...
    const summary = {
      ...
      overdueSubmittals: submittals.filter(
        (s) =>
          s.dueDate &&
          new Date(s.dueDate) < todayStart &&
          s.status !== "APPROVED",
      ).length,
      ...
      totalUrgentItems:
        tasks.filter((t) => t.priority === "CRITICAL" || t.priority === "HIGH")
          .length +
