...
export function EquipmentClient({ equipment, projects }: EquipmentClientProps) {
      ...
      const res = await fetch("/api/equipment", {
        ...
        body: JSON.stringify({
          ...
          purchaseCost: newEquipment.purchaseCost
            ? parseFloat(newEquipment.purchaseCost)
            : null,
        }),
      });
      if (!res.ok) {
  ...
  const stats = {
    ...
    serviceDue: equipment.filter(
      (e) =>
        e.nextServiceDate &&
        differenceInDays(new Date(e.nextServiceDate), new Date()) <= 7,
    ).length,
  };
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Manage construction equipment and tools
          </p>
        </div>
        <Button
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredEquipment.map((item) => {
            ...
            const serviceDueSoon =
              ...
              differenceInDays(new Date(item.nextServiceDate), new Date()) <= 7;
              ...
              >
                <CardContent className="p-5">
                  ...
                  <div className="space-y-2 text-sm text-slate-600 dark:text-slate-400">
                    ...
                    {item.nextServiceDate && (
                      <p
                        className={`flex items-center gap-2 ${serviceDueSoon ? "text-orange-600" : ""}`}
                      >
                    ...
                    {item.status === "AVAILABLE" && (
                      <Select
                        onValueChange={(projectId) =>
                          handleStatusUpdate(item.id, "IN_USE", projectId)
                        }
                      >
                    ...
                    {item.status !== "MAINTENANCE" &&
                      item.status !== "OUT_OF_SERVICE" && (
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            handleStatusUpdate(item.id, "MAINTENANCE")
                          }
                        >
