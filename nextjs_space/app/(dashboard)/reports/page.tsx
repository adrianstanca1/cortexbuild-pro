...
export default async function ReportsPage() {
  ...
  const stats = {
    ...
    activeProjects: projects.filter(
      (p: ProjectWithTasks) => p.status === "IN_PROGRESS",
    ).length,
    completedProjects: projects.filter(
      (p: ProjectWithTasks) => p.status === "COMPLETED",
    ).length,
    ...
    totalBudget: projects.reduce(
      (sum: number, p: ProjectWithTasks) => sum + (p.budget || 0),
      0,
    ),
  };
  ...
  const projectsByStatus = {
    PLANNING: projects.filter((p: ProjectWithTasks) => p.status === "PLANNING")
      .length,
    IN_PROGRESS: projects.filter(
      (p: ProjectWithTasks) => p.status === "IN_PROGRESS",
    ).length,
    ON_HOLD: projects.filter((p: ProjectWithTasks) => p.status === "ON_HOLD")
      .length,
  ...
  const projectPerformance = projects.map((p: ProjectWithTasks) => ({
    ...
    progress:
      p._count.tasks > 0
        ? Math.round(
            (p.tasks.filter((t: Task) => t.status === "COMPLETE").length /
              p._count.tasks) *
              100,
          )
        : 0,
