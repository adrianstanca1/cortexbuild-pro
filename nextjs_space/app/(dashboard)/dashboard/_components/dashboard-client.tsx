...
export function DashboardClient({ 
  ...
  useRealtimeSubscription(
    ['task_created', 'task_updated', 'project_created', 'project_updated', 'team_member_added', 'document_uploaded', 'rfi_created', 'submittal_created', 'safety_incident_reported'],
  ...
  if (constructionMetrics?.failedInspections && constructionMetrics.failedInspections > 0) {
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <Badge variant="outline" className="hidden sm:flex items-center gap-1 text-xs bg-gradient-to-r from-primary/10 to-purple-500/10 border-primary/30 text-primary font-medium">
              <Sparkles className="h-3 w-3" /> AI-Enhanced
            </Badge>
          </div>
          <p className="text-slate-600 dark:text-slate-300 mt-1">Your construction portfolio at a glance</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="relative overflow-hidden border-slate-200 dark:border-slate-700 hover:shadow-lg transition-all duration-300 group bg-white dark:bg-slate-800">
          ...
          <CardContent className="p-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">Active Projects</p>
                <p className="text-3xl font-bold text-slate-800 dark:text-slate-100 mt-1">{stats?.totalProjects ?? 0}</p>
                <div className="flex items-center gap-2 mt-2">
                  <Badge variant="info" className="text-xs font-medium">{projectStatusCounts.IN_PROGRESS} in progress</Badge>
                </div>
              </div>
        ...
        <Card className="relative overflow-hidden border-slate-200 dark:border-slate-700 hover:shadow-lg transition-all duration-300 group bg-white dark:bg-slate-800">
          ...
          <CardContent className="p-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">Active Tasks</p>
                <p className="text-3xl font-bold text-slate-800 dark:text-slate-100 mt-1">{stats?.activeTasks ?? 0}</p>
                <div className="flex items-center gap-2 mt-2">
                  {criticalTasks > 0 && (
                  ...
                  {urgentTasks > 0 && !criticalTasks && (
                    <Badge variant="warning" className="text-xs font-medium">{urgentTasks} due today</Badge>
                  )}
                  {!criticalTasks && !urgentTasks && (
                    <Badge variant="success" className="text-xs font-medium">On track</Badge>
                  )}
                </div>
              </div>
        ...
        <Card className="relative overflow-hidden border-slate-200 dark:border-slate-700 hover:shadow-lg transition-all duration-300 group bg-white dark:bg-slate-800">
          ...
          <CardContent className="p-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">Team Members</p>
                <p className="text-3xl font-bold text-slate-800 dark:text-slate-100 mt-1">{stats?.teamMembers ?? 0}</p>
                <div className="flex items-center gap-2 mt-2">
                  <Badge variant="outline" className="text-xs font-medium text-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-600">Active this week</Badge>
                </div>
              </div>
        ...
        <Card className="relative overflow-hidden border-slate-200 dark:border-slate-700 hover:shadow-lg transition-all duration-300 group bg-white dark:bg-slate-800">
          ...
          <CardContent className="p-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">Safety Status</p>
                ...
                <div className="flex items-center gap-2 mt-2">
                  {constructionMetrics?.criticalIncidents === 0 ? (
                    <Badge variant="success" className="text-xs font-medium">No critical issues</Badge>
                  ) : (
                    <Badge variant="destructive" className="text-xs font-medium">Needs attention</Badge>
                  )}
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                ...
                <div className="flex flex-col items-center justify-center">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-300 mt-4 text-center font-medium">
                    {projectStatusCounts.COMPLETED} of {totalProjects} projects completed
                  </p>
                </div>
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-700">
                {projects?.slice(0, 5)?.map((project) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-semibold text-slate-800 dark:text-slate-100 group-hover:text-primary transition-colors">{project.name}</p>
                ...
                {(!projects || projects.length === 0) && (
                  <div className="p-8 text-center">
                    ...
                    <p className="text-slate-600 dark:text-slate-300 font-medium">No projects yet</p>
                    <Link href="/projects/new">
        ...
        <div className="space-y-6">
          ...
          <Card className="border-slate-200 dark:border-slate-700 overflow-hidden bg-white dark:bg-slate-800">
            ...
            <CardContent className="space-y-2">
              <QuickAction icon={<Plus className="h-5 w-5" />} label="New Task" href="/tasks" variant="primary" />
              <QuickAction icon={<FileQuestion className="h-5 w-5" />} label="Submit RFI" href="/rfis" />
              <QuickAction icon={<ClipboardCheck className="h-5 w-5" />} label="Daily Report" href="/daily-reports" />
              <QuickAction icon={<Shield className="h-5 w-5" />} label="Safety Check" href="/safety" />
              <QuickAction icon={<HardHat className="h-5 w-5" />} label="Site Diary" href="/site-diary" />
            </CardContent>
          </Card>
          ...
          {constructionMetrics && (
            <>
              <KPICard
                ...
                variant={constructionMetrics.criticalIncidents > 0 ? "alert" : "default"}
                metrics={[
                  { label: "Safety Incidents (30d)", value: constructionMetrics.safetyIncidentsThisMonth, status: constructionMetrics.safetyIncidentsThisMonth > 0 ? "danger" : "success" },
                  { label: "Critical Incidents", value: constructionMetrics.criticalIncidents, status: constructionMetrics.criticalIncidents > 0 ? "danger" : "success" },
                  { label: "Failed Inspections", value: constructionMetrics.failedInspections, status: constructionMetrics.failedInspections > 0 ? "warning" : "success" },
                  { label: "Open Punch Items", value: constructionMetrics.openPunchItems, status: "neutral" }
                ]}
              ...
              <KPICard
                ...
                metrics={[
                  { label: "Open RFIs", value: constructionMetrics.openRFIs, status: constructionMetrics.openRFIs > 5 ? "warning" : "neutral" },
                  { label: "Overdue RFIs", value: constructionMetrics.overdueRFIs, status: constructionMetrics.overdueRFIs > 0 ? "danger" : "success" },
                  { label: "Pending Submittals", value: constructionMetrics.pendingSubmittals, status: "neutral" },
                  { label: "Upcoming Inspections", value: constructionMetrics.upcomingInspections, status: "info" }
                ]}
              ...
              <KPICard
                ...
                metrics={[
                  { label: "Total Budget", value: formatCurrency(constructionMetrics.totalBudget), status: "neutral" },
                  { label: "Pending Change Orders", value: constructionMetrics.pendingChangeOrders, status: constructionMetrics.pendingChangeOrders > 0 ? "warning" : "success" },
                  { label: "Approved CO Value", value: formatCurrency(constructionMetrics.changeOrderValue), status: "neutral" }
                ]}
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-slate-100 dark:divide-slate-700 max-h-80 overflow-y-auto">
                {activities?.slice(0, 8)?.map((activity) => (
                  <div key={activity.id} className="px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-600 dark:text-slate-300 mt-0.5 font-medium">
                          {activity.project?.name || 'System'} • {formatDistanceToNow(new Date(activity.createdAt), { addSuffix: true })}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {(!activities || activities.length === 0) && (
                  <div className="p-6 text-center">
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-300 font-medium">No recent activity</p>
                  </div>
                )}
              </div>
            </CardContent>
      ...
      <Card className="border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
        <CardHeader className="border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
          <div className="flex items-center justify-between">
            ...
            <Link href="/tasks" className="text-sm text-primary hover:text-primary/80 font-semibold flex items-center gap-1">
              View All <ChevronRight className="h-4 w-4" />
            </Link>
          </div>
        </CardHeader>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/30">
                  <th className="text-left text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider px-4 py-3">Task</th>
                  <th className="text-left text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider px-4 py-3">Project</th>
                  <th className="text-left text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider px-4 py-3">Assignee</th>
                  <th className="text-left text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider px-4 py-3">Due Date</th>
                  <th className="text-left text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider px-4 py-3">Priority</th>
                  <th className="text-left text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider px-4 py-3">Status</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={task.id} className={`hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors ${
                      ...
                      <td className="px-4 py-3">
                        <Link href={`/tasks`} className="font-semibold text-slate-800 dark:text-slate-100 hover:text-primary">
                          {task.title}
                        </Link>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-sm text-slate-700 dark:text-slate-300 font-medium">{task.project?.name}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-sm text-slate-700 dark:text-slate-300 font-medium">{task.assignee?.name || 'Unassigned'}</span>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <Badge variant={priorityColors[task.priority as keyof typeof priorityColors] || 'secondary'} className="font-medium">
                          {task.priority}
                        </Badge>
                      </td>
                      <td className="px-4 py-3">
                        <Badge variant={statusColors[task.status as keyof typeof statusColors] || 'secondary'} className="font-medium">
            ...
            {(!tasks || tasks.length === 0) && (
              <div className="p-8 text-center">
                ...
                <p className="text-slate-600 dark:text-slate-300 font-medium">No tasks yet</p>
                <Link href="/tasks">
                  <Button variant="outline" size="sm" className="mt-3">
