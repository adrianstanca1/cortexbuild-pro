  ...
  const constructionMetrics = {
    ...
    openRFIs:
      rfisLoading || rfisError
        ? 0
        : (rfis?.filter(
            (r: any) => r?.status === "OPEN" || r?.status === "DRAFT",
          )?.length ?? 0),
    overdueRFIs:
      rfisLoading || rfisError
        ? 0
    ...
    // Submittal metrics
    pendingSubmittals:
      submittalsLoading || submittalsError
        ? 0
        : (submittals?.filter(
            (s: any) =>
    ...
    // Safety metrics
    safetyIncidentsThisMonth:
      safetyIncidentsLoading || safetyIncidentsError
        ? 0
        : (safetyIncidents?.length ?? 0),
    criticalIncidents:
      safetyIncidentsLoading || safetyIncidentsError
        ? 0
        : (safetyIncidents?.filter(
            (i: any) => i?.severity === "CRITICAL" || i?.severity === "HIGH",
          )?.length ?? 0),

    // Punch list metrics
    openPunchItems:
      punchListsLoading || punchListsError
        ? 0
        : (punchLists?.filter(
    ...
    criticalPunchItems:
      punchListsLoading || punchListsError
        ? 0
        : (punchLists?.filter(
            (p: any) => p?.priority === "CRITICAL" && p?.status !== "COMPLETED",
          )?.length ?? 0),
    ...
    failedInspections:
      inspectionsLoading || inspectionsError
        ? 0
        : (inspections?.filter((i: any) => i?.status === "FAILED")?.length ??
          0),

    // Change order metrics
    pendingChangeOrders:
      changeOrdersLoading || changeOrdersError
        ? 0
        : (changeOrders?.filter(
            (c: any) =>
    ...
    changeOrderValue:
      changeOrdersLoading || changeOrdersError
        ...
        : (changeOrders
            ...
            ?.reduce(
              (sum: number, c: any) =>
                sum + (Number((c as any)?.costImpact) || 0),
              0,
            ) ?? 0),
            ...
            <div className="space-x-3">
              {projectsError && (
                <button
                  ...
                  className="px-3 py-1 bg-indigo-50 text-indigo-600 text-xs rounded"
                >
                  Projects
                </button>
              )}
              ...
              {safetyIncidentsError && (
                <button
                  onClick={refetchSafetyIncidents}
                  className="px-3 py-1 bg-indigo-50 text-indigo-600 text-xs rounded"
                >
                  Safety
                </button>
              ...
              <div className="h-4 w-full bg-blue-200 rounded-md relative overflow-hidden">
                <div
                  ...
                  style={{
                    width: Math.min(
                      (projectStatusCounts.PLANNING /
                        Math.max(1, stats.totalProjects)) *
                        100,
                      100,
                    ),
                  }}
              ...
              <div className="p-3 bg-yellow-50 rounded-lg border">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium text-red-600">
                    {constructionMetrics.overdueRFIs}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="grid gap-3">
              <div className="p-3 bg-green-50 rounded-lg border">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">
                    {constructionMetrics.safetyIncidentsThisMonth}
                  </span>
                </div>
              </div>
            ...
            <div className="grid gap-3">
              <div className="p-3 bg-purple-50 rounded-lg border">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">
                    {constructionMetrics.pendingChangeOrders}
                  </span>
                </div>
              </div>
              <div className="p-3 bg-green-50 rounded-lg border">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">
                    $
                    {constructionMetrics.changeOrderValue?.toLocaleString() ??
                      "0"}
                  </span>
                </div>
              </div>
            ...
            <div className="grid gap-3">
              <div className="p-3 bg-blue-50 rounded-lg border">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">
                    {constructionMetrics.upcomingInspections}
                  </span>
                </div>
              </div>
              <div className="p-3 bg-yellow-50 rounded-lg border">
              ...
              <div className="p-3 bg-green-50 rounded-lg border">
                <div className="flex_items-center justify-between">
                  ...
                  <span className="font-medium">
                    {constructionMetrics.openPunchItems}
                  </span>
                </div>
              </div>
              <div className="p-3 bg-red-50 rounded-lg border">
                <div className="flex_items-center justify-between">
                  ...
                  <span className="font-medium text-red-600">
                    {constructionMetrics.criticalPunchItems}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="p-6 bg-white rounded-lg border shadow-sm">
          ...
          <div className="space-y-4">
            {activities?.map((activity) => (
              <div
                key={activity.id}
                className="p-3 border-l-4 border-blue-500 pl-4"
              >
