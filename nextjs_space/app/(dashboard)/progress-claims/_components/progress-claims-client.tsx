...
export function ProgressClaimsClient({
  claims: initialClaims,
  projects,
}: ProgressClaimsClientProps) {
  ...
  const filteredClaims = claims.filter((claim) => {
    const matchesSearch = claim.project?.name
      ?.toLowerCase()
      .includes(search.toLowerCase());
    const matchesStatus =
      statusFilter === "all" || claim.status === statusFilter;
    const matchesProject =
      projectFilter === "all" || claim.projectId === projectFilter;
  ...
  const handleCreateClaim = async () => {
    if (
      !newClaim.projectId ||
      !newClaim.claimPeriodFrom ||
      !newClaim.claimPeriodTo
    ) {
                  ...
                  <div className="flex gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mb-3">
                        Period:{" "}
                        {format(new Date(claim.claimPeriodFrom), "MMM d, yyyy")}{" "}
                        - {format(new Date(claim.claimPeriodTo), "MMM d, yyyy")}
                      </p>
                      ...
                      <div className="grid grid-cols-4 gap-6 text-sm">
                        <div>
                          ...
                          <p className="font-semibold">
                            £{claim.previousClaimed?.toLocaleString() || "0"}
                          </p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">This Claim</p>
                          <p className="font-semibold text-blue-600">
                            £{claim.thisClaim?.toLocaleString() || "0"}
                          </p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Retention</p>
                          <p className="font-semibold text-orange-600">
                            -£{claim.retentionHeld?.toLocaleString() || "0"}
                          </p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Net Payable</p>
                          <p className="font-bold text-green-600">
                            £{claim.netPayable?.toLocaleString() || "0"}
                          </p>
                        </div>
                      </div>
                    </div>
          ...
          <div className="grid gap-4 py-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setNewClaim({ ...newClaim, projectId: v })
                }
              >
                <SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewClaim({
                      ...newClaim,
                      claimPeriodFrom: e.target.value,
                    })
                  }
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewClaim({ ...newClaim, claimPeriodTo: e.target.value })
                  }
                />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewClaim({ ...newClaim, thisClaim: e.target.value })
                  }
                  placeholder="0.00"
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewClaim({ ...newClaim, retentionHeld: e.target.value })
                  }
                  placeholder="0.00"
                />
              </div>
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setNewClaim({ ...newClaim, notes: e.target.value })
                }
