...
export function ResourceAllocation() {
  ...
  const [sortBy, setSortBy] = useState<"utilization" | "tasks" | "name">(
    "utilization",
  );
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(
        "/api/dashboard/analytics?type=resource-allocation",
      );
  ...
  const getUtilizationBadge = (rate: number) => {
    if (rate >= 100)
      return { label: "Overloaded", variant: "destructive" as const };
  ...
  const teamSummary = {
    ...
    optimal: resources.filter(
      (r) => r.utilizationRate >= 80 && r.utilizationRate < 100,
    ).length,
    available: resources.filter((r) => r.utilizationRate < 50).length,
    avgUtilization:
      resources.length > 0
        ? Math.round(
            resources.reduce((acc, r) => acc + r.utilizationRate, 0) /
              resources.length,
          )
        : 0,
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p
                  className={`text-2xl font-bold ${getUtilizationColor(teamSummary.avgUtilization)}`}
                >
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {teamSummary.totalHours.toFixed(0)}h
                </p>
              </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant={utilBadge.variant}>
                          {utilBadge.label}
                        </Badge>
                      </div>
                      <p className="text-sm text-muted-foreground">
                    ...
                    <div className="flex-shrink-0 text-right space-y-1">
                      <div className="flex items-center gap-2 justify-end">
                        ...
                        <span
                          className={`font-bold ${getUtilizationColor(resource.utilizationRate)}`}
                        >
                    ...
                    {resource.criticalTasks > 0 && (
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-orange-600">
                          {resource.criticalTasks} critical
                        </span>
                      </div>
                    )}
                  </div>
