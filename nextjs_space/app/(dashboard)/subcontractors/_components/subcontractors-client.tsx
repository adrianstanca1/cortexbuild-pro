...
export function SubcontractorsClient({
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        "Are you sure you want to delete this subcontractor? This will also delete all associated contracts.",
      )
    )
      return;
    try {
      ...
      {filteredSubcontractors.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredSubcontractors.map((sub) => {
            const tradeConfig =
              TRADES.find((t) => t.value === sub.trade) || TRADES[0];
            const TradeIcon = tradeConfig.icon;
            const activeContracts = sub.contracts.filter(
              (c) => c.status === "ACTIVE",
            ).length;
            const totalValue = sub.contracts.reduce(
              (sum, c) => sum + c.contractAmount,
              0,
            );
            const isInsuranceExpiring =
              sub.insuranceExpiry &&
              new Date(sub.insuranceExpiry) <
                  ...
                  <CardContent className="p-4">
                    ...
                    {isInsuranceExpiring && (
                      <div className="flex items-center gap-2 text-xs text-amber-600 bg-amber-50 p-2 rounded mb-3">
                        ...
                        Insurance expires{" "}
                        {sub.insuranceExpiry &&
                          format(new Date(sub.insuranceExpiry), "MMM d, yyyy")}
                      </div>
                    )}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewItem({ ...newItem, licenseNumber: e.target.value })
                  }
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewItem({ ...newItem, insuranceExpiry: e.target.value })
                  }
                />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setSelectedItem({ ...selectedItem, trade: v })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {TRADES.find((t) => t.value === selectedItem.trade)?.label}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  {renderStars(selectedItem.rating ?? undefined) || (
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {selectedItem.insuranceExpiry
                      ? format(
                          new Date(selectedItem.insuranceExpiry),
                          "MMM d, yyyy",
                        )
                      : "-"}
                  </p>
