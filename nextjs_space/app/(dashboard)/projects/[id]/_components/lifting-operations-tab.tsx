...
export function LiftingOperationsTab({
  project,
  teamMembers,
  liftingOperations: initialOps,
}: LiftingOperationsTabProps) {
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage crane lifts and lifting plans
          </p>
        </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold text-yellow-600">
              {operations.filter((o) => o.status === "PLANNED").length}
            </div>
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap items-center gap-4 mt-3 text-sm text-muted-foreground">
                      ...
                      <span>
                        📅 {format(new Date(op.liftDate), "dd/MM/yyyy HH:mm")}
                      </span>
                      <span className="flex items-center gap-1">
                        <Weight className="h-4 w-4" /> {op.loadWeight} kg
                      </span>
                      {op.craneType && <span>{op.craneType}</span>}
                      {op.liftRadius && (
                        <span className="flex items-center gap-1">
                          <Ruler className="h-4 w-4" /> {op.liftRadius}m radius
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-4 mt-2 text-sm">
                      {op.liftPlanAttached && (
                        <Badge variant="outline" className="text-green-600">
                          <Check className="h-3 w-3 mr-1" /> Lift Plan
                        </Badge>
                      )}
                      {op.exclusionZoneSet && (
                        <Badge variant="outline" className="text-green-600">
                          <Check className="h-3 w-3 mr-1" /> Exclusion Zone
                        </Badge>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    {op.status === "PLANNED" && (
                      <Button
                        ...
                        onClick={() => handleStatusChange(op.id, "APPROVED")}
                      >
                        Approve
                      </Button>
                    )}
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Truck className="h-5 w-5 text-blue-500" /> New Lifting Operation
              Plan
            </DialogTitle>
            <DialogDescription>
              Create a detailed lift plan for crane operations
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-6 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    value={form.liftDate}
                    onChange={(e) =>
                      setForm((prev) => ({ ...prev, liftDate: e.target.value }))
                    }
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    value={form.loadCog}
                    onChange={(e) =>
                      setForm((prev) => ({ ...prev, loadCog: e.target.value }))
                    }
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setForm((prev) => ({
                        ...prev,
                        exclusionZoneSet: !!checked,
                      }))
                    }
                  />
                  <label
                    htmlFor="exclusionZone"
                    className="text-sm text-foreground cursor-pointer"
                  >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    value={form.banksman}
                    onChange={(e) =>
                      setForm((prev) => ({ ...prev, banksman: e.target.value }))
                    }
                    placeholder="Name"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setForm((prev) => ({ ...prev, operatorId: v }))
                    }
                  >
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setForm((prev) => ({ ...prev, supervisorId: v }))
                    }
                  >
                    <SelectTrigger>
            ...
            <div>
              ...
              <SignaturePad
                onSave={(sig) =>
                  setForm((prev) => ({ ...prev, plannerSignature: sig }))
                }
                onClear={() =>
                  setForm((prev) => ({ ...prev, plannerSignature: "" }))
                }
              />
            </div>
          </div>
      ...
      <Dialog open={showDetailModal} onOpenChange={setShowDetailModal}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedOp && (
            <div className="space-y-6 py-4">
              <div className="flex items-center gap-2">
                <Badge className={statusColors[selectedOp.status]}>
                  {selectedOp.status.replace("_", " ")}
                </Badge>
                <span className="text-muted-foreground">
                  {format(new Date(selectedOp.liftDate), "dd/MM/yyyy HH:mm")}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    <p>
                      ...
                      {selectedOp.loadDescription}
                    </p>
                    <p>
                      <span className="font-medium">Weight:</span>{" "}
                      {selectedOp.loadWeight} kg
                    </p>
                    {selectedOp.loadDimensions && (
                      <p>
                ...
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    {selectedOp.craneType && (
                      <p>
                        <span className="font-medium">Type:</span>{" "}
                        {selectedOp.craneType}
                      </p>
                    )}
                    {selectedOp.craneCapacity && (
                      <p>
                        <span className="font-medium">Capacity:</span>{" "}
                        {selectedOp.craneCapacity} tonnes
                      </p>
                    )}
                    {selectedOp.craneMake && (
                      <p>
                ...
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    {selectedOp.liftRadius && (
                      <p>
                        <span className="font-medium">Radius:</span>{" "}
                        {selectedOp.liftRadius}m
                      </p>
                    )}
                    {selectedOp.liftHeight && (
                      <p>
                        <span className="font-medium">Height:</span>{" "}
                        {selectedOp.liftHeight}m
                      </p>
                    )}
                    {selectedOp.windSpeedLimit && (
                      <p>
                        <span className="font-medium">Max Wind:</span>{" "}
                        {selectedOp.windSpeedLimit} km/h
                      </p>
                    )}
                  </div>
                </div>
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    <p>
                      ...
                      {selectedOp.plannedBy?.name}
                    </p>
                    {selectedOp.operator && (
                      <p>
                        <span className="font-medium">Operator:</span>{" "}
                        {selectedOp.operator?.name}
                      </p>
                    )}
                    {selectedOp.supervisor && (
                      <p>
                        <span className="font-medium">Supervisor:</span>{" "}
                        {selectedOp.supervisor?.name}
                      </p>
                    )}
                    {selectedOp.banksman && (
                      <p>
                        <span className="font-medium">Banksman:</span>{" "}
                        {selectedOp.banksman}
                      </p>
                    )}
                  </div>
              ...
              <div className="flex gap-4">
                {selectedOp.liftPlanAttached && (
                  <Badge variant="outline" className="text-green-600">
                    <Check className="h-3 w-3 mr-1" /> Lift Plan Attached
                  </Badge>
                )}
                {selectedOp.exclusionZoneSet && (
                  <Badge variant="outline" className="text-green-600">
                    <Check className="h-3 w-3 mr-1" /> Exclusion Zone Set
                  </Badge>
                )}
              </div>
            </div>
