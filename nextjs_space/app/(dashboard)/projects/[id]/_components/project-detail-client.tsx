          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setNewRfi({ ...newRfi, assignedToId: v })}
              >
                <SelectTrigger>
    ...
    switch (activeTab) {
      ...
      case "milestones":
        return (
          <MilestonesTab
            project={project}
            onCreateMilestone={() => openCreateModal("milestone")}
          />
        );
      case "time-tracking":
        return (
          <TimeTrackingTab
            project={project}
            onCreateEntry={() => openCreateModal("time-entry")}
          />
        );
      case "budget":
      ...
      case "rfis":
        return (
          <RFIsTab
            project={project}
            onCreateRfi={() => openCreateModal("rfi")}
          />
        );
      case "submittals":
      ...
      case "toolbox-talks":
        return (
          <ToolboxTalksTab
            projectId={project?.id}
            toolboxTalks={project?.toolboxTalks || []}
      ...
      case "daily-checks":
        return (
          <DailyChecksTab
            projectId={project?.id}
            mewpChecks={project?.mewpChecks || []}
            toolChecks={project?.toolChecks || []}
            equipment={[]}
          />
        );
      case "risk-assessments":
        return (
          <RiskAssessmentsTab
            project={project}
            teamMembers={availableTeamMembers}
            riskAssessments={project?.riskAssessments || []}
          />
        );
      case "permits-to-work":
        return (
          <PermitsToWorkTab
            project={project}
            teamMembers={availableTeamMembers}
            hotWorkPermits={project?.hotWorkPermits || []}
            confinedSpacePermits={project?.confinedSpacePermits || []}
          />
        );
      case "lifting-ops":
        return (
          <LiftingOperationsTab
            project={project}
            teamMembers={availableTeamMembers}
            liftingOperations={project?.liftingOperations || []}
          />
        );
      case "site-access":
        return (
          <SiteAccessTab
            project={project}
            teamMembers={availableTeamMembers}
            siteAccessLogs={project?.siteAccessLogs || []}
          />
        );
      case "certifications":
        return (
              ...
              {category.items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {count > 0 && (
                      <span
                        className={`text-xs px-1.5 py-0.5 rounded-full ${activeTab === item.id ? "bg-white/20" : "bg-gray-200"}`}
                      >
                        {count}
                      </span>
                    )}
...
function OverviewTab({
  ...
  const completedTasks =
    project?.tasks?.filter((t: any) => t?.status === "COMPLETE")?.length || 0;
  const totalTasks = project?.tasks?.length || 0;
  const taskProgress =
    totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0;
  const totalBudget = project?.budget || 0;
  const spentBudget =
    project?.costItems?.reduce(
      (sum: number, item: any) => sum + (item?.actualCost || 0),
      0,
    ) || 0;
  const budgetUsed =
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500">
              {budgetUsed}% of £{totalBudget.toLocaleString()}
            </p>
            <div className="mt-2 h-2 bg-gray-100 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full ${budgetUsed > 90 ? "bg-red-500" : budgetUsed > 70 ? "bg-yellow-500" : "bg-blue-500"}`}
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {project?.rfis?.filter((r: any) => r?.status !== "CLOSED")
                    ?.length || 0}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Submittals:</span>
                <span className="font-medium">
                  {project?.submittals?.filter(
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {project?.defects?.filter((d: any) => d?.status !== "CLOSED")
                    ?.length || 0}
                </span>
              </div>
              <div className="flex justify-between">
            ...
            <Card key={m?.id}>
              <CardContent className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div
                    className={`w-3 h-3 rounded-full ${m?.status === "COMPLETED" ? "bg-green-500" : m?.status === "IN_PROGRESS" ? "bg-blue-500" : "bg-gray-300"}`}
                  />
                  <div>
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Total: {totalHours.toFixed(1)} hours logged
          </p>
        </div>
...
function BudgetTab({ project }: { project: any }) {
  ...
  const totalActual = costItems.reduce(
    (sum: number, i: any) => sum + (i?.actualCost || 0),
    0,
  );
              ...
              <CardContent className="p-4 flex justify-between items-center">
                <div>
                  <p className="font-medium">
                    Claim #{c?.claimNumber || c?.id?.slice(-6)}
                  </p>
                  <p className="text-sm text-gray-500">
                    {c?.periodStart && format(new Date(c.periodStart), "MMM d")}{" "}
                    -{" "}
                    {c?.periodEnd &&
          ...
          {orders.map((co: any) => (
            <Card key={co?.id}>
              <CardContent className="p-4 flex justify-between items-center">
                <div>
                  <p className="font-medium">
                    {co?.title || `CO #${co?.number || co?.id?.slice(-6)}`}
                  </p>
                  <p className="text-sm text-gray-500">
                ...
                <div className="text-right">
                  <p
                    className={`font-bold ${(co?.costChange || 0) >= 0 ? "text-red-600" : "text-green-600"}`}
                  >
                    {(co?.costChange || 0) >= 0 ? "+" : ""}£
                    {Math.abs(co?.costChange || 0).toLocaleString()}
                  </p>
                  <Badge
                    variant={
                      co?.status === "APPROVED"
          ...
          {teamMembers.map((tm: any) => (
            <Card key={tm?.id}>
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium">
                    {tm?.teamMember?.user?.name || "Unknown"}
                  </p>
                  <p className="text-sm text-gray-500">
                    {tm?.teamMember?.user?.email}
                  </p>
                  <Badge variant="secondary" className="mt-1">
                    {tm?.role || tm?.teamMember?.user?.role || "Member"}
                  </Badge>
                </div>
              </CardContent>
            </Card>
          ...
          {subcontractors.map((s: any) => (
            <Card key={s?.id}>
              <CardContent className="p-4 flex justify-between items-center">
                <div>
                  <p className="font-medium">
                    {s?.subcontractor?.companyName || s?.companyName}
                  </p>
                  <p className="text-sm text-gray-500">
                    {s?.scope || s?.subcontractor?.trade}
                  </p>
                </div>
                <div className="text-right">
                  <p className="font-bold">
                    £{(s?.contractValue || 0).toLocaleString()}
                  </p>
                  <Badge
          ...
          {submittals.map((s: any) => (
            <Card key={s?.id}>
              <CardContent className="p-4 flex justify-between items-center">
                <div>
                  <p className="font-medium">
                    {s?.title || `Submittal #${s?.number}`}
                  </p>
                  <p className="text-sm text-gray-500">
                    {s?.type} • By {s?.submittedBy?.name || "Unknown"}
                  </p>
                </div>
                <Badge
                  variant={
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <Button>
          <Plus className="h-4 w-4 mr-2" /> Upload Drawing
        </Button>
      </div>
      {drawings.length === 0 ? (
        <Card>
            ...
            <Card key={d?.id}>
              <CardContent className="p-4 flex justify-between items-center">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium">
                      {d?.title || d?.drawingNumber}
                    </p>
                    <p className="text-sm text-gray-500">
                      Rev {d?.revision || "0"} • {d?.discipline}
                    </p>
                  </div>
                </div>
                <Badge
                  variant={d?.status === "APPROVED" ? "success" : "secondary"}
                >
                  {d?.status}
                </Badge>
              </CardContent>
            ...
            <Card key={r?.id}>
              <CardContent className="p-4 flex justify-between items-center">
                <div>
                  <p className="font-medium">
                    {r?.reportDate &&
                      format(new Date(r.reportDate), "EEEE, MMMM d, yyyy")}
                  </p>
                  <p className="text-sm text-gray-500">
                    By {r?.createdBy?.name || "Unknown"} • Weather:{" "}
                    {r?.weather || "N/A"} • Crew: {r?.manpowerCount || 0}
                  </p>
                </div>
        ...
        <div className="space-y-2">
          {diaries.map((d: any) => (
            <Card key={d?.id}>
              <CardContent className="p-4">
                <p className="font-medium">
                  {d?.date && format(new Date(d.date), "MMMM d, yyyy")}
                </p>
          ...
          {defects.map((d: any) => (
            <Card key={d?.id}>
              <CardContent className="p-4 flex justify-between items-center">
                <div>
                  <p className="font-medium">
                    {d?.title || d?.description?.slice(0, 50)}
                  </p>
                  <p className="text-sm text-gray-500">
                    {d?.location} • Reported{" "}
                    {d?.createdAt &&
                ...
                <div className="flex items-center gap-2">
                  <Badge
                    variant={
                      priorityColors[
                        d?.severity as keyof typeof priorityColors
                      ] || "secondary"
                    }
                  >
                    {d?.severity || "MEDIUM"}
...
function PunchListsTab({ project }: { project: any }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold">
          Punch Lists ({punchLists.length})
        </h2>
        <Button>
          <Plus className="h-4 w-4 mr-2" /> New Punch List
        </Button>
      </div>
      {punchLists.length === 0 ? (
        <Card>
          ...
          {punchLists.map((pl: any) => (
            <Card key={pl?.id}>
              <CardContent className="p-4 flex justify-between items-center">
                <div>
                  <p className="font-medium">
                    {pl?.title || `Punch List #${pl?.id?.slice(-6)}`}
                  </p>
                  <p className="text-sm text-gray-500">
                    {pl?.location} • {pl?.items?.length || 0} items
                  </p>
                </div>
                <Badge
                  variant={pl?.status === "COMPLETE" ? "success" : "warning"}
                >
                  {pl?.status}
                </Badge>
              </CardContent>
              ...
              <CardContent className="p-4 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    ...
                    {i?.incidentDate &&
                      format(new Date(i.incidentDate), "MMM d, yyyy")}
                  </p>
                </div>
                <Badge
                  variant={
