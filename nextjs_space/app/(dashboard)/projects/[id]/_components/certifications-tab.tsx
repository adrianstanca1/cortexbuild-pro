...
const certificationTypes = [
  ...
  },
  {
    value: "CPCS_CARD",
    label: "CPCS Card",
    description: "Plant Operator Certificate",
  },
  {
    value: "IPAF_LICENSE",
    label: "IPAF License",
    description: "MEWP/Cherry Picker Operator",
  },
  {
    value: "PASMA_CERTIFICATE",
    label: "PASMA",
    description: "Mobile Tower Scaffold",
  },
  ...
  {
    value: "ASBESTOS_AWARENESS",
    label: "Asbestos Awareness",
    description: "Category A Asbestos",
  },
  {
    value: "MANUAL_HANDLING",
    label: "Manual Handling",
    description: "Safe Lifting Techniques",
  },
  {
    value: "WORKING_AT_HEIGHT",
    label: "Working at Height",
    description: "Height Safety Training",
  },
  {
    value: "CONFINED_SPACE",
    label: "Confined Space",
    description: "CS Entry Training",
  },
  {
    value: "HOT_WORK",
    label: "Hot Work",
    description: "Fire Prevention Training",
  },
  {
    value: "SLINGER_SIGNALLER",
    label: "Slinger/Signaller",
    description: "Crane Slinger Certificate",
  },
  {
    value: "CRANE_OPERATOR",
    label: "Crane Operator",
    description: "Crane Operation License",
  },
  {
    value: "EXCAVATOR_OPERATOR",
    label: "Excavator Operator",
    description: "Plant Machine Operation",
  },
  {
    value: "FORKLIFT_OPERATOR",
    label: "Forklift Operator",
    description: "Industrial Truck License",
  },
  {
    value: "ELECTRICIAN_LICENSE",
    label: "Electrician License",
    description: "Electrical Competence",
  },
...
export function CertificationsTab({
  ...
  const getExpiryStatus = (cert: any) => {
    if (cert.isLifetime)
      return { status: "valid", label: "Lifetime", color: "bg-green-500" };
    if (!cert.expiryDate)
      return { status: "unknown", label: "No Expiry", color: "bg-gray-500" };
  ...
  const expiredCount = certifications.filter(
    (c) => getExpiryStatus(c).status === "expired",
  ).length;
  const expiringCount = certifications.filter((c) =>
    ["expiring", "warning"].includes(getExpiryStatus(c).status),
  ).length;
      ...
      const response = await fetch("/api/certifications", {
        ...
        body: JSON.stringify({
          ...
          certificationName:
            ...
            certificationTypes.find((t) => t.value === form.certificationType)
              ?.label,
        }),
      });
  ...
  const groupedByWorker: Record<string, WorkerGroup> = filteredCerts.reduce(
    (acc: Record<string, WorkerGroup>, cert: any) => {
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Track training and qualification records
          </p>
        </div>
              ...
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  <div className="font-bold text-red-600">
                    {expiredCount} Expired Certifications
                  </div>
            ...
            <div className="text-2xl font-bold text-green-600">
              {
                certifications.filter(
                  (c) => getExpiryStatus(c).status === "valid",
                ).length
              }
            </div>
        ...
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Award className="h-5 w-5 text-primary" /> Add Certification
            </DialogTitle>
            <DialogDescription>
              Record a worker's certification or training
            </DialogDescription>
          </DialogHeader>
