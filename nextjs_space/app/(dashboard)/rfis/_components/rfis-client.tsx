...
const statusConfig: Record<string, { color: string; bgColor: string; icon: React.ReactNode; label: string }> = {
  ...
  OPEN: { color: 'text-blue-600', bgColor: 'bg-blue-100 dark:bg-blue-900/30', icon: <Send className="w-3.5 h-3.5" />, label: 'Open' },
  ANSWERED: { color: 'text-emerald-600', bgColor: 'bg-emerald-100 dark:bg-emerald-900/30', icon: <CheckCircle className="w-3.5 h-3.5" />, label: 'Answered' },
  CLOSED: { color: 'text-purple-600', bgColor: 'bg-purple-100 dark:bg-purple-900/30', icon: <CheckCircle className="w-3.5 h-3.5" />, label: 'Closed' }
};

export function RFIsClient({ initialRFIs, projects, teamMembers }: RFIsClientProps) {
  ...
  const handleAnswerRFI = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setRFIs(rfis.map(r => r.id === updated.id ? { ...r, ...updated } : r));
  ...
  const filteredRFIs = rfis.filter(rfi => {
    ...
    const matchesProject = projectFilter === 'all' || rfi.projectId === projectFilter;
  ...
  const stats = {
    ...
    overdue: rfis.filter(r => r.dueDate && isPast(new Date(r.dueDate)) && r.status === 'OPEN').length,
    dueSoon: rfis.filter(r => r.dueDate && !isPast(new Date(r.dueDate)) && isWithinInterval(new Date(r.dueDate), { start: new Date(), end: addDays(new Date(), 7) }) && r.status === 'OPEN').length,
    withCostImpact: rfis.filter(r => r.costImpact && r.status !== 'CLOSED').length
  };
  ...
  const getUrgencyIndicator = (rfi: RFI) => {
    ...
    if (rfi.dueDate && isWithinInterval(new Date(rfi.dueDate), { start: new Date(), end: addDays(new Date(), 3) })) return 'urgent';
    return null;
  };
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">Track RFIs and responses across projects</p>
        </div>
        <Button onClick={() => setShowNewModal(true)} className="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 shadow-lg shadow-purple-500/25">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Total RFIs</p>
              </div>
          ...
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-blue-600/70 dark:text-blue-400/70 font-medium">Open</p>
              </div>
          ...
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-emerald-600/70 dark:text-emerald-400/70 font-medium">Answered</p>
              </div>
          ...
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-red-600/70 dark:text-red-400/70 font-medium">Overdue</p>
              </div>
          ...
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-amber-600/70 dark:text-amber-400/70 font-medium">Due Soon</p>
              </div>
        ...
        <Card className="bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/30 dark:to-orange-800/30 border-0 shadow-md hover:shadow-lg transition-all">
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">{stats.withCostImpact}</p>
                <p className="text-xs text-orange-600/70 dark:text-orange-400/70 font-medium">Cost Impact</p>
              </div>
      ...
      {filteredRFIs.length === 0 ? (
        <Card className="border-0 shadow-md">
          <CardContent className="py-16 text-center">
            ...
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-2">No RFIs found</h3>
            <p className="text-slate-500 dark:text-slate-400 mb-4">Get started by creating your first RFI</p>
            <Button onClick={() => setShowNewModal(true)} className="bg-purple-600 hover:bg-purple-700">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        <Badge className={`${statusConfig[rfi.status].bgColor} ${statusConfig[rfi.status].color} border-0`}>
                          {statusConfig[rfi.status].icon}
                          <span className="ml-1">{statusConfig[rfi.status].label}</span>
                        </Badge>
                        {rfi.costImpact && (
                          <Badge variant="outline" className="text-orange-600 border-orange-300 bg-orange-50 dark:bg-orange-900/20">
                            <PoundSterling className="w-3 h-3 mr-1" />Cost Impact
                          </Badge>
                        )}
                        {rfi.scheduleImpact && (
                          <Badge variant="outline" className="text-red-600 border-red-300 bg-red-50 dark:bg-red-900/20">
                            <Timer className="w-3 h-3 mr-1" />Schedule Impact
                          </Badge>
                        )}
                        {urgency === 'overdue' && (
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-1 mt-1">{rfi.question}</p>
                      <div className="flex flex-wrap items-center gap-3 mt-3 text-xs text-slate-500 dark:text-slate-400">
                        ...
                        {rfi._count.attachments > 0 && (
                          <span className="flex items-center gap-1.5">
                            <Paperclip className="w-3.5 h-3.5" />{rfi._count.attachments}
                          </span>
                        )}
                      </div>
                    </div>
            ...
            return (
              ...
              >
                <CardContent className="p-5">
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 mb-4">{rfi.question}</p>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {rfi.costImpact && (
                      <Badge variant="outline" className="text-orange-600 border-orange-300 text-xs">
                        <PoundSterling className="w-3 h-3 mr-1" />Cost
                      </Badge>
                    )}
                    {rfi.scheduleImpact && (
                      <Badge variant="outline" className="text-red-600 border-red-300 text-xs">
                        <Timer className="w-3 h-3 mr-1" />Schedule
                      </Badge>
                    )}
                    {urgency === 'overdue' && (
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-4">
            <div>
              ...
              <Select value={newRFI.projectId} onValueChange={(v) => setNewRFI({ ...newRFI, projectId: v })}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNewRFI({ ...newRFI, specSection: e.target.value })}
                  placeholder="e.g., 03 30 00"
                />
              </div>
              <div>
                <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Drawing Reference</label>
                <Input
                  className="mt-1.5"
            ...
            <div className="flex gap-6 p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) => setNewRFI({ ...newRFI, costImpact: e.target.checked })}
                  className="rounded border-slate-300 text-purple-600 focus:ring-purple-500"
                />
                <span className="text-sm text-slate-700 dark:text-slate-300">Cost Impact</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={newRFI.scheduleImpact}
                  onChange={(e) => setNewRFI({ ...newRFI, scheduleImpact: e.target.checked })}
                  className="rounded border-slate-300 text-purple-600 focus:ring-purple-500"
                />
                <span className="text-sm text-slate-700 dark:text-slate-300">Schedule Impact</span>
              </label>
            </div>
            <div className="flex justify-end gap-3 pt-4 border-t">
              <Button variant="outline" onClick={() => setShowNewModal(false)}>Cancel</Button>
              <Button onClick={handleCreateRFI} disabled={loading} className="bg-purple-600 hover:bg-purple-700">
          ...
          {selectedRFI && (
            <>
              <DialogHeader>
                <div className="flex items-center justify-between">
                  <DialogTitle className="flex items-center gap-3">
                    ...
                    <Badge className={`${statusConfig[selectedRFI.status].bgColor} ${statusConfig[selectedRFI.status].color} border-0`}>
              ...
              <div className="space-y-5 mt-4">
                <div>
                  <h3 className="text-xl font-semibold text-slate-900 dark:text-white">{selectedRFI.subject}</h3>
                ...
                <div className="grid grid-cols-2 gap-4 p-4 bg-slate-50 dark:bg-slate-800 rounded-lg text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-slate-900 dark:text-white">{selectedRFI.createdBy.name}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <User className="w-4 h-4 text-slate-400" />
                    <span className="text-slate-500">Assigned To:</span>
                    <span className="font-medium text-slate-900 dark:text-white">{selectedRFI.assignedTo?.name || 'Unassigned'}</span>
                  </div>
                  {selectedRFI.specSection && (
                    <div className="flex items-center gap-2">
                      <FileQuestion className="w-4 h-4 text-slate-400" />
                      <span className="text-slate-500">Spec Section:</span>
                      <span className="font-medium text-slate-900 dark:text-white">{selectedRFI.specSection}</span>
                    </div>
                  )}
                  {selectedRFI.drawingRef && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-slate-900 dark:text-white">{selectedRFI.drawingRef}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="border-t pt-5">
                  ...
                  <div className="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-lg border border-purple-200 dark:border-purple-800">
                    <p className="whitespace-pre-wrap text-slate-700 dark:text-slate-300">{selectedRFI.question}</p>
                  </div>
                </div>
                ...
                {selectedRFI.answer && (
                  <div className="border-t pt-5">
                    ...
                    <div className="bg-emerald-50 dark:bg-emerald-900/20 p-4 rounded-lg border border-emerald-200 dark:border-emerald-800">
                      <p className="whitespace-pre-wrap text-slate-700 dark:text-slate-300">{selectedRFI.answer}</p>
                    </div>
                  </div>
                )}
                ...
                {selectedRFI.status === 'OPEN' && (
                  <div className="border-t pt-5">
                    <h4 className="font-semibold text-slate-900 dark:text-white mb-3">Provide Answer</h4>
                    <Textarea
                      value={answerInput}
                    ...
                    <div className="flex justify-end gap-3 mt-3">
                      <Button onClick={handleAnswerRFI} disabled={loading || !answerInput.trim()} className="bg-emerald-600 hover:bg-emerald-700">
                ...
                {selectedRFI.status === 'ANSWERED' && (
                  <div className="flex justify-end pt-4 border-t">
                    <Button onClick={handleCloseRFI} disabled={loading} variant="outline">
