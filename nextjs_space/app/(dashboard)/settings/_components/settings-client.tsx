        ...
        <div className="flex-1 min-w-0">
            ...
            <div className="space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-end gap-4 -mt-12">
                    ...
                    <div className="flex-1 min-w-0 pb-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Badge className="bg-primary/10 text-primary border-0">{roleLabel}</Badge>
              ...
              <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
                ...
                <div className="space-y-4">
                  {[
                    { label: "Create Projects", allowed: user?.role !== "FIELD_WORKER" },
                    { label: "Manage Team", allowed: user?.role === "ADMIN" || user?.role === "PROJECT_MANAGER" || user?.role === "COMPANY_OWNER" || user?.role === "SUPER_ADMIN" },
                    { label: "Delete Projects", allowed: user?.role === "ADMIN" || user?.role === "COMPANY_OWNER" || user?.role === "SUPER_ADMIN" },
                    ...
                    <div key={i} className="flex items-center justify-between py-3 border-b border-slate-100 dark:border-slate-700 last:border-0">
                      <span className="text-slate-600 dark:text-slate-400">{perm.label}</span>
                      <span className={`flex items-center gap-1.5 text-sm font-medium ${perm.allowed ? 'text-green-600 dark:text-green-400' : 'text-slate-400'}`}>
                        {perm.allowed ? <><Check className="h-4 w-4" /> Enabled</> : "Disabled"}
                      </span>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === "security" && (
            <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">Update your password to keep your account secure</p>
              ...
              <div className="max-w-md space-y-5">
                ...
                <div>
                  ...
                  <Input
                    ...
                    className="h-11 rounded-xl border-slate-200 dark:border-slate-700"
                  />
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1.5">Must be at least 8 characters</p>
                </div>
                <div>
          ...
          {activeTab === "notifications" && (
            <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">Choose how you want to be notified</p>
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <div className="space-y-4">
                    {[
                      { key: "emailTaskAssigned", label: "Task Assignments", desc: "Get notified when a task is assigned to you" },
                      { key: "emailProjectUpdates", label: "Project Updates", desc: "Receive updates about projects you're part of" },
                      { key: "emailDailyDigest", label: "Daily Digest", desc: "Get a daily summary of all activity" },
                    ].map((item) => (
                      <div key={item.key} className="flex items-center justify-between py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-800/50">
                        <div>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {[
                      { key: "pushTaskReminders", label: "Task Reminders", desc: "Get reminded about upcoming due dates" },
                      { key: "pushComments", label: "Comments & Mentions", desc: "Get notified when someone comments or mentions you" },
                    ].map((item) => (
                      <div key={item.key} className="flex items-center justify-between py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-800/50">
                        <div>
                          <p className="font-medium text-sm text-slate-900 dark:text-white">{item.label}</p>
          ...
          {activeTab === "organization" && (
            <div className="space-y-6">
                ...
                <div className="px-6 pb-6">
                  <div className="flex items-end gap-4 -mt-10">
                    <div className="h-20 w-20 rounded-2xl bg-gradient-to-br from-primary to-purple-600 flex items-center justify-center text-white text-3xl font-bold border-4 border-white dark:border-slate-800 shadow-xl">
                      {(organization?.name ?? "O")?.charAt(0)?.toUpperCase() ?? "O"}
                    </div>
                    <div className="pb-1">
                      <h2 className="text-xl font-bold text-slate-900 dark:text-white">{organization?.name ?? "Your Organization"}</h2>
                      <p className="text-sm text-slate-500 dark:text-slate-400">@{organization?.slug ?? "organization"}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-6">Organization Details</h3>
                <div className="space-y-4">
                  {[
                    { label: "Organization ID", value: <code className="text-xs bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded-lg text-slate-600 dark:text-slate-300">{organization?.id ?? "N/A"}</code> },
                    { label: "Your Role", value: <Badge className="bg-primary/10 text-primary border-0">{roleLabel}</Badge> },
                ...
                {(user?.role === "ADMIN" || user?.role === "COMPANY_OWNER" || user?.role === "SUPER_ADMIN") && (
                  <div className="mt-6 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl">
                    <p className="text-sm text-amber-800 dark:text-amber-200">
                      <strong>Admin Note:</strong> As an administrator, you can manage organization settings, 
                      invite team members, and configure integrations from the Team page.
                    </p>
                  </div>
                )}
              </div>
