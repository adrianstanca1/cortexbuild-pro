...
const TRADES = [
  ...
  "FRAMING", "DRYWALL", "PAINTING", "FLOORING", "LANDSCAPING", "OTHER"
];

const STATUSES = ["IDENTIFIED", "ASSIGNED", "IN_PROGRESS", "COMPLETED", "VERIFIED", "DISPUTED"];
...
export function DefectsClient({ defects: initialDefects, projects, _teamMembers }: DefectsClientProps) {
  ...
  const stats = {
    ...
    open: defects.filter(d => ["IDENTIFIED", "ASSIGNED", "IN_PROGRESS"].includes(d.status)).length,
    completed: defects.filter(d => d.status === "COMPLETED" || d.status === "VERIFIED").length,
    critical: defects.filter(d => d.priority === "CRITICAL" && d.status !== "VERIFIED").length,
  };
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between">
                  <div className="flex gap-4">
                    <div className={`p-2 rounded-lg ${getPriorityColor(defect.priority)} bg-opacity-20`}>
                      <AlertTriangle className={`h-5 w-5 ${defect.priority === 'CRITICAL' ? 'text-red-600' : 'text-orange-600'}`} />
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-sm text-muted-foreground">#{defect.number}</span>
                        <h3 className="font-semibold" style={{ color: "#1a1a1a" }}>{defect.title}</h3>
                        <Badge className={getStatusColor(defect.status)}>{defect.status.replace("_", " ")}</Badge>
                      ...
                      <div className="flex flex-wrap gap-3 mt-2 text-xs text-muted-foreground">
                        ...
                        {defect.dueDate && (
                          <span>Due: {format(new Date(defect.dueDate), "MMM d, yyyy")}</span>
                        )}
                      </div>
                    </div>
                  </div>
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select value={newDefect.projectId} onValueChange={(v) => setNewDefect({ ...newDefect, projectId: v })}>
              ...
              <div>
                ...
                <Select value={newDefect.trade} onValueChange={(v) => setNewDefect({ ...newDefect, trade: v })}>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNewDefect({ ...newDefect, floor: e.target.value })}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNewDefect({ ...newDefect, room: e.target.value })}
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNewDefect({ ...newDefect, responsibleParty: e.target.value })}
