...
export function SubmittalsClient({
  ...
  const [selectedSubmittal, setSelectedSubmittal] = useState<Submittal | null>(
    null,
  );
  ...
  const filteredSubmittals = submittals.filter((submittal) => {
    ...
    const matchesStatus =
      statusFilter === "all" || submittal.status === statusFilter;
    const matchesProject =
      projectFilter === "all" || submittal.projectId === projectFilter;
  ...
  const stats = {
    ...
    needsRevision: submittals.filter((s) => s.status === "REVISE_RESUBMIT")
      .length,
  ...
  const getUrgencyIndicator = (submittal: Submittal) => {
    if (submittal.status === "APPROVED" || submittal.status === "REJECTED")
      return null;
    if (submittal.dueDate && isPast(new Date(submittal.dueDate)))
      return "overdue";
    if (
      submittal.dueDate &&
      isWithinInterval(new Date(submittal.dueDate), {
        start: new Date(),
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Track material and product submittal approvals
          </p>
        </div>
        <Button
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        <Badge
                          className={`${statusConfig[submittal.status].bgColor} ${statusConfig[submittal.status].color} border-0`}
                        >
                        ...
                        {submittal.revisionNumber > 0 && (
                          <Badge
                            variant="outline"
                            className="text-slate-600 border-slate-300 bg-slate-50 dark:bg-slate-800"
                          >
                      ...
                      <div className="flex flex-wrap items-center gap-3 mt-3 text-xs text-slate-500 dark:text-slate-400">
                        ...
                        <span className="flex items-center gap-1.5">
                          ...
                          {formatDistanceToNow(new Date(submittal.createdAt), {
                            addSuffix: true,
                          })}
                        </span>
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4 mt-4">
            <div>
              ...
              <Select
                value={newSubmittal.projectId}
                onValueChange={(v) =>
                  setNewSubmittal({ ...newSubmittal, projectId: v })
                }
              >
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewSubmittal({ ...newSubmittal, title: e.target.value })
                }
              ...
              <DialogHeader>
                <div className="flex items-center justify-between">
                  <DialogTitle className="flex items-center gap-3">
                    ...
                    <Badge
                      className={`${statusConfig[selectedSubmittal.status].bgColor} ${statusConfig[selectedSubmittal.status].color} border-0`}
                    >
                      {statusConfig[selectedSubmittal.status].label}
                    </Badge>
                    {selectedSubmittal.revisionNumber > 0 && (
                      <Badge variant="outline">
                        Rev {selectedSubmittal.revisionNumber}
                      </Badge>
                    )}
                  </DialogTitle>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 p-4 bg-slate-50 dark:bg-slate-800 rounded-lg text-sm">
                  ...
                  {selectedSubmittal.dueDate && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-slate-900 dark:text-white">
                        {format(
                          new Date(selectedSubmittal.dueDate),
                          "MMM d, yyyy",
                        )}
                      </span>
                    </div>
                  ...
                  {selectedSubmittal.submittedBy && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-slate-900 dark:text-white">
                        {selectedSubmittal.submittedBy.name}
                      </span>
                    </div>
                  )}
                  {selectedSubmittal.reviewedBy && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-slate-900 dark:text-white">
                        {selectedSubmittal.reviewedBy.name}
                      </span>
                    </div>
                  )}
                </div>
