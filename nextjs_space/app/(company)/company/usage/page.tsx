...
export default async function UsagePage() {
  ...
  const totalDocumentSize = documents.reduce((sum: number, doc: { fileSize: number | null }) => sum + (doc.fileSize || 0), 0);
  const documentsByType = documents.reduce((acc: Record<string, number>, doc: { documentType: string }) => {
  ...
  const usageData = {
    ...
    documentsByType,
    totalDocumentSize,
    projectsByStatus: projectsByStatus.reduce((acc: Record<string, number>, p: { status: string; _count: { status: number } }) => {
