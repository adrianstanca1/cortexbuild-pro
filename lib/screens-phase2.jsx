...
(function() {
  ...
  if (_seedDirty) { try { localStorage.setItem('cortexx_db_v1', JSON.stringify(snap)); } catch (e) {} }
  ...
  const makeT = (name) => ({
    ...
    update: async (id, patch) => {
      const s = Backend.db.snapshot();
      s[name] = arr(name).map(x => x.id == id ? {...x, ...patch} : x);
      try { localStorage.setItem('cortexx_db_v1', JSON.stringify(s)); } catch (e) {}
      Backend.db.user.update({});
          ...
          right={<button onClick={() => {
            ...
            if (window.cortexxReportPDF) {
              window.cortexxReportPDF({
                title: rep.l || 'Business report',
                kpis: [
                  { l: 'Cash', v: `£${cash.toLocaleString('en-GB', {maximumFractionDigits: 0})}` },
                  { l: 'Outstanding', v: `£${outstanding.toLocaleString()}` },
                  { l: 'Pipeline', v: `£${(pipeline/1000).toFixed(0)}k` },
                  { l: 'Avg margin', v: `${margin.toFixed(1)}%` },
                ],
                narrative: narrative || 'Generate the AI narrative first for a fuller report.',
              });
            } else { window.print(); }
          }} style={{
