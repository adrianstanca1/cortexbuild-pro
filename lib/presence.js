...
(function () {
  ...
  function peerId() {
    ...
    if (!v) { v = 'p_' + Math.random().toString(36).slice(2, 10); sessionStorage.setItem(k, v); }
    return v;
  }
  function myIdentity() {
    var name = 'You', avatar = null;
    try {
      var u = (window.Backend && Backend.db && Backend.db.user && Backend.db.user.getSync) ? Backend.db.user.getSync() : null;
      if (u) { name = u.name || u.email || 'You'; avatar = u.avatar || null; }
  ...
  // ── State ─────────────────────────────────────────────────────
  var me = myIdentity();
  var state = { screen: 'overview', focus: null };       // what I'm viewing
  var peers = {};                                          // id → { identity, screen, focus, at }
  var subs = new Set();
  var bc = null, transport = null;

  try { bc = ('BroadcastChannel' in window) ? new BroadcastChannel(CHANNEL) : null; } catch (e) { bc = null; }

  function now() { return Date.now(); }
  function notify() { subs.forEach(function (fn) { try { fn(); } catch (e) {} }); }

  function frame(type) {
    return { type: type, id: me.id, identity: me, screen: state.screen, focus: state.focus, at: now() };
  }
  function send(type) {
    var f = frame(type);
    try { if (bc) bc.postMessage(f); } catch (e) {}
    try { if (transport && transport.send) transport.send(f); } catch (e) {}
  }

  function ingest(f) {
    if (!f || !f.id || f.id === me.id) return;
    if (f.type === 'leave') { if (peers[f.id]) { delete peers[f.id]; notify(); } return; }
    var existing = peers[f.id];
    peers[f.id] = { identity: f.identity || (existing && existing.identity) || { id: f.id, name: 'Someone', color: '#888' }, screen: f.screen, focus: f.focus, at: f.at || now() };
    // A newly-seen peer should learn about us promptly.
    if (!existing) send('hello');
    notify();
  }

  if (bc) bc.onmessage = function (e) { ingest(e.data); };

  // ── Heartbeat + stale sweep ───────────────────────────────────
  var hb = setInterval(function () { send('beat'); }, HEARTBEAT_MS);
  var sweep = setInterval(function () {
    var t = now(), changed = false;
    for (var id in peers) { if (t - peers[id].at > STALE_MS) { delete peers[id]; changed = true; } }
    if (changed) notify();
  }, SWEEP_MS);
  ...
  window.CortexPresence = {
    ...
    count: function () { return Object.keys(peers).length; },
    ...
    _stop: function () { clearInterval(hb); clearInterval(sweep); send('leave'); },
  };

  // React hook: live presence for a screen/focus, re-renders on change.
  window.usePresence = function (screen, focus) {
    var R = window.React;
    var ref = R.useReducer(function (x) { return x + 1; }, 0);
    var force = ref[1];
