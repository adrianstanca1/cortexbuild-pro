...
const WorkspaceChip = ({ accent = T.blue }) => {
  ...
  return (
    <button onClick={() => window.cortexxNav && window.cortexxNav('switchworkspace')} title="Switch workspace" style={{
      ...
      {React.cloneElement(Ic.chevDown, { size: 12, color: T.t3 })}
    </button>
  );
};

// Shared chrome — every variation gets these so they feel like the same app.
function MobileHeader({ title, subtitle, accent = T.blue, right, ws }) {
  const rightContent = ws
    ? <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}><WorkspaceChip accent={accent}/>{right}</div>
    : right;
  return (
    <div style={{
  ...
  <button onClick={onClick} style={{
    ...
    {count > 0 ? (
      <span style={{
        ...
        borderRadius: 8, background: T.red, color: '#fff', fontFamily: SF, fontSize: 10, fontWeight: 800,
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        border: `1.5px solid ${T.bg0}`, boxSizing: 'border-box', lineHeight: 1,
