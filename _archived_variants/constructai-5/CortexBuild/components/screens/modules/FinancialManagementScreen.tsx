...
import { ToolDefinition, FINANCIAL_MANAGEMENT_TOOLS } from '../../../tool-definitions.ts';
import ToolCard from '../../shared/ToolCard.tsx';
import { ChevronLeftIcon } from '../../Icons.tsx';

interface FinancialManagementScreenProps {
    currentUser: User;
...
const FinancialManagementScreen: React.FC<FinancialManagementScreenProps> = ({ currentUser, navigateTo, goBack, openProjectSelector, onDeepLink, can }) => {
    const visibleTools = FINANCIAL_MANAGEMENT_TOOLS.filter(tool => 
        ...
        <div className="flex flex-col h-full">
            <header className="bg-white p-4 flex items-center border-b mb-8">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-500">Oversee company-wide financial health.</p>
                </div>
            </header>
            <main className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {visibleTools.map(tool => (
                 ...
                 {visibleTools.length === 0 && (
                    <p className="text-gray-500 md:col-span-2 lg:col-span-3">You do not have access to any financial management tools.</p>
                )}
            </main>
        </div>
    );
