...
import { ToolDefinition, TIME_TRACKING_TOOLS } from '../../../tool-definitions.ts';
...
const TimeTrackingScreen: React.FC<TimeTrackingScreenProps> = ({ currentUser, navigateTo, goBack, openProjectSelector, onDeepLink, can }) => {
    const visibleTools = TIME_TRACKING_TOOLS.filter(tool => 
        ...
        <div className="flex flex-col h-full">
            <header className="bg-white p-4 flex items-center border-b mb-8">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-500">Manage timesheets and labor costs.</p>
                </div>
            </header>
            <main className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {visibleTools.map(tool => (
                 ...
                 {visibleTools.length === 0 && (
                    <p className="text-gray-500 md:col-span-2 lg:col-span-3">You do not have access to any time tracking tools.</p>
                )}
            </main>
        </div>
    );
