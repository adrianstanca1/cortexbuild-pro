...
export const AIAdvisor: React.FC<AIAdvisorProps> = ({ user, addToast, onBack }) => {
    ...
    const initializeChat = useCallback(async () => {
        ...
        try {
            ...
            const projectContext = projData.map((p: any) => `Project: ${p.name} (Status: ${p.status}, Budget: £${p.budget})`).join('\n');
            // FIX: Corrected property from .text to .title
            const taskContext = todoData.slice(0, 10).map((t: any) => `Task: ${t.text || t.title} (Status: ${t.status}, Priority: ${t.priority})`).join('\n');
            ...
            setHistory([{ role: 'model', text: initialResponse.text || 'Hello! I am Ash, your AI construction advisor. How can I help you today?' }]);
        } catch (error) {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            setHistory(prev => [...prev, { role: 'model', text: result.text || 'I received your message but could not generate a response.' }]);
        } catch (error) {
