...
import { Play, Pause, RotateCcw, Coffee, Loader2, BarChart3 } from 'lucide-react';
import toast from 'react-hot-toast';
import { pomodoroService } from '../../../lib/services/marketplaceAppsService';
...
const PomodoroTimerApp: React.FC<PomodoroTimerAppProps> = ({ isDarkMode = true, currentUser }) => {
    ...
    const loadStats = async () => {
        ...
        try {
            setLoading(true);
            const data = await pomodoroService.getStats(currentUser.id);
            setStats(data);
        } catch (error) {
    ...
    const handleTimerComplete = async () => {
        ...
        if (!isBreak && currentUser?.id && currentSessionId) {
            // Complete work session
            try {
                await pomodoroService.complete(currentSessionId);
    ...
    const toggleTimer = async () => {
        if (!isActive && !isBreak && currentUser?.id) {
            // Starting a new work session
            try {
                const session = await pomodoroService.start(currentUser.id, 25, 'work');
        ...
        <div className={`min-h-full ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'} p-8 flex items-center justify-center`}>
            <div className="max-w-2xl w-full">
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                    <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl p-4 text-center`}>
                        <BarChart3 className={`h-6 w-6 mx-auto mb-2 ${isDarkMode ? 'text-purple-400' : 'text-purple-600'}`} />
                    ...
                    <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl p-4 text-center`}>
                        <Coffee className={`h-6 w-6 mx-auto mb-2 ${isDarkMode ? 'text-green-400' : 'text-green-600'}`} />
                        <div className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                            {stats.totalMinutes}
                        </div>
                        <div className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                ...
                <div className="flex justify-center gap-4 mb-8">
                    <button
                        type="button"
                        onClick={toggleTimer}
                        className={`p-6 rounded-full ${isBreak
                    ...
                    <button
                        type="button"
                        onClick={resetTimer}
                        className={`p-6 rounded-full ${isDarkMode ? 'bg-gray-800 hover:bg-gray-700' : 'bg-white hover:bg-gray-100'
