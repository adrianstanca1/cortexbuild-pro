...
export const SettingsView: React.FC<SettingsViewProps> = ({ user, addToast, theme, setTheme }) => {
    ...
    const handleThemeChange = async (newTheme: 'light' | 'dark') => {
        setTheme(newTheme);
        try {
            await api.updateCompanySettings(user.companyId, { theme: newTheme });
    ...
    const handleNotificationChange = async (key: keyof NotificationPreferences, value: boolean) => {
        ...
        setSettings({ ...settings, notificationPreferences: newPrefs });
        
        try {
            await api.updateCompanySettings(user.companyId, { notificationPreferences: { [key]: value } });
    ...
    const handleSaveCompanySettings = async (e: React.FormEvent) => {
        ...
        if (!settings) return;
        try {
            await api.updateCompanySettings(user.companyId, { timesheetRetentionDays: settings.timesheetRetentionDays });
    ...
    const navItems = [
        ...
        ...(isAdmin ? [{ id: 'company', label: 'Company', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg> }] : []),
        { id: 'notifications', label: 'Notifications', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg> },
