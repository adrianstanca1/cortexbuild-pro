    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const [companiesData, allUsers, metricsData] = await Promise.all([
                api.getCompanies(),
                api.getUsersByCompany(),
                api.getPlatformUsageMetrics(),
            ]);
            
            const tenantCompanies = companiesData.filter((c: any) => c.id !== 0);
            setCompanies(tenantCompanies);
            setTotalUsers(allUsers.filter((u: any) => u.companyId !== 0).length);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card>
                     <h3 className="font-semibold text-lg mb-4">Platform Usage Metrics</h3>
                        ...
                        {metrics.map(metric => (
                            <div key={metric.name} className="p-3 bg-slate-50 rounded-lg">
                                ...
                                <p className="text-2xl font-bold text-slate-800">
                                    ...
                                    <span className="text-sm font-medium text-slate-500 ml-1.5">{metric.unit}</span>
                                </p>
                            </div>
                        ))}
                     </div>
