...
export const ProjectDetailView: React.FC<ProjectDetailViewProps> = ({ project, user, onBack, addToast, isOnline, onStartChat }) => {
    ...
    const [allPersonnel, setAllPersonnel] = useState<User[]>([]);
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            setAllPersonnel(allUsers);

            const projectAssignments = allAssignments.filter((a: any) => a.projectId === project.id);
            const teamMembers = allUsers.filter((u: any) => projectAssignments.some((pa: any) => pa.userId === u.id));
            setTeam(teamMembers);
    ...
    const handleUpdateTask = async (taskId: number | string, updates: Partial<Todo>) => {
        try {
            ...
            const newTodos = todos.map(t => t.id === taskId ? updatedTask : t)
            setTodos(newTodos);
    ...
    const mapMarkers: MapMarker[] = useMemo(() => [
        { type: 'project', lat: project.location.lat, lng: project.location.lng, radius: project.geofenceRadius, popupContent: project.name }
    ], [project]);
        ...
        <div className="space-y-6">
            ...
            {loading ? <Card><p>Loading project data...</p></Card> : (
                <>
                    ...
                    {activeTab === 'tasks' && <KanbanBoard todos={todos} allTodos={todos} onUpdateTodo={handleUpdateTask} personnel={team} user={user} onTaskClick={(task) => setSelectedTodo(task)} />}
