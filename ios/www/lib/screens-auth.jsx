...
(function () {
  const inp = () => ({ width: '100%', boxSizing: 'border-box', background: T.bg2, border: `0.5px solid ${T.hairMid}`, borderRadius: 11, padding: '12px 13px', color: T.t1, fontFamily: SF, fontSize: 14.5, outline: 'none', marginTop: 6 });
  const lbl = { fontFamily: SF, fontSize: 12.5, color: T.t2, fontWeight: 600 };

  function Field({ label, type, value, onChange, placeholder, autoFocus }) {
    return React.createElement('div', null,
      React.createElement('div', { style: lbl }, label),
      React.createElement('input', { style: inp(), type: type || 'text', value, onChange: (e) => onChange(e.target.value), placeholder, autoFocus }));
  }
  ...
  async function localSession(name, email, company) {
    try {
      if (window.Backend) {
        await Backend.db.user.update({ name: name || email.split('@')[0], email, ...(company ? { companyBrief: company } : {}) });
      }
      localStorage.setItem('cortexx_session', JSON.stringify({ email, name, local: true, at: Date.now() }));
  ...
  function AccountSheet({ onClose, accent = T.blue }) {
    ...
    const live = hasApi();
    const session = (() => { try { return JSON.parse(localStorage.getItem('cortexx_session') || 'null'); } catch (e) { return null; } })();
    ...
    React.useEffect(() => {
      if (inviteToken && live) {
        setMode('invite');
        cortexxCloud.inviteInfo(inviteToken).then((info) => { if (info && info.workspace) setInviteMeta(info); });
    ...
    const go = async () => {
      ...
      let ok = false;
      if (mode === 'signin') {
        if (live) ok = await cortexxCloud.loginPassword(email, password);
        else ok = await localSession(name, email);
      } else if (mode === 'register') {
        if (live) ok = await cortexxCloud.register(name, email, password, company);
        else ok = await localSession(name, email, company);
      } else if (mode === 'invite') {
        if (live) ok = await cortexxCloud.acceptInvite(inviteToken, name, password);
        else { setErr('Accepting an invite needs the cloud API — connect in Settings → Cloud sync first.'); setBusy(false); return; }
      }
      setBusy(false);
      if (ok) {
        localStorage.setItem('cortexx_session', JSON.stringify({ email: email || (inviteMeta && inviteMeta.email), name, local: !live, at: Date.now() }));
        onClose();
    ...
    return React.createElement(SheetWrap, { title: 'Account', onClose, accent },
      React.createElement('div', { style: { padding: '4px 16px 28px', display: 'flex', flexDirection: 'column', gap: 14 } },
        // mode badge
        React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8 } },
          React.createElement('span', { style: { width: 7, height: 7, borderRadius: 7, background: live ? T.green : T.amber } }),
          React.createElement('span', { style: { fontFamily: SF, fontSize: 12, color: T.t3 } },
            live ? 'Connected to cloud — full multi-tenant accounts' : 'No cloud API configured — accounts are stored on this device only')),
        // current session
        (session || cloudAuthed) && React.createElement('div', { style: { background: T.bg2, border: `0.5px solid ${T.hair}`, borderRadius: 12, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 10 } },
          React.createElement('div', { style: { width: 34, height: 34, borderRadius: 17, background: accent, color: '#fff', display: 'grid', placeItems: 'center', fontFamily: SF, fontWeight: 800, fontSize: 14 } }, ((session && (session.name || session.email)) || 'U')[0].toUpperCase()),
        ...
        mode === 'signin' && React.createElement(React.Fragment, null,
          !live && React.createElement(Field, { label: 'Your name', value: name, onChange: setName, placeholder: 'Adrian Stanca', autoFocus: true }),
          React.createElement(Field, { label: 'Email', type: 'email', value: email, onChange: setEmail, placeholder: 'you@company.com', autoFocus: live }),
          live && React.createElement(Field, { label: 'Password', type: 'password', value: password, onChange: setPassword, placeholder: '••••••••' })),
        mode === 'register' && React.createElement(React.Fragment, null,
          React.createElement(Field, { label: 'Your name', value: name, onChange: setName, placeholder: 'Adrian Stanca', autoFocus: true }),
          React.createElement(Field, { label: 'Company / workspace', value: company, onChange: setCompany, placeholder: 'Stanca Construction Ltd' }),
          React.createElement(Field, { label: 'Email', type: 'email', value: email, onChange: setEmail, placeholder: 'you@company.com' }),
          live && React.createElement(Field, { label: 'Password', type: 'password', value: password, onChange: setPassword, placeholder: 'min 8 characters' })),
        mode === 'invite' && React.createElement(React.Fragment, null,
          inviteMeta && React.createElement('div', { style: { background: T.green + '14', border: `0.5px solid ${T.green}44`, borderRadius: 11, padding: '11px 13px', fontFamily: SF, fontSize: 13, color: T.t1 } },
            'Invited to ', React.createElement('strong', null, inviteMeta.workspace), ' as ', inviteMeta.role, ' (', inviteMeta.email, ')'),
          React.createElement(Field, { label: 'Invite code', value: inviteToken, onChange: setInviteToken, placeholder: 'paste your invite code or open the invite link' }),
          React.createElement(Field, { label: 'Your name', value: name, onChange: setName, placeholder: 'Adrian Stanca' }),
          React.createElement(Field, { label: 'Choose a password', type: 'password', value: password, onChange: setPassword, placeholder: 'min 8 characters' })),
        err && React.createElement('div', { style: { background: T.red + '14', border: `0.5px solid ${T.red}44`, borderRadius: 10, padding: '10px 13px', fontFamily: SF, fontSize: 12.5, color: T.red } }, err),
        React.createElement(PrimaryBtn, { accent, disabled: busy || (mode === 'signin' ? !emailOk || (live && !password) : mode === 'register' ? !name.trim() || !emailOk || (live && password.length < 8) : !inviteToken.trim() || !name.trim() || password.length < 8), onClick: go },
          busy ? 'Working…' : (mode === 'signin' ? 'Sign in' : mode === 'register' ? 'Create workspace' : 'Accept invite')),
        !live && mode !== 'invite' && React.createElement('div', { style: { fontFamily: SF, fontSize: 11.5, color: T.t3, lineHeight: 1.5 } },
  ...
  function InviteTeammateSheet({ onClose, accent = T.blue }) {
    const [email, setEmail] = React.useState('');
    ...
    const send = async () => {
      ...
      if (live) {
        ...
        if (r && r.link) { setResult({ link: r.link, live: true }); setPending(await cortexxCloud.listInvites().then(x => Array.isArray(x) ? x.filter(i => !i.used) : [])); }
      } else {
        // Honest local fallback: record the intent, give a shareable placeholder
        if (window.Backend) {
          try { await Backend.db.team.create({ n: email.split('@')[0], role: role, email, status: 'invited' }); } catch (e) {}
        }
        setResult({ link: null, live: false });
        toast('Added to team as invited (device only)', 'success');
      }
      setBusy(false);
    };

    return React.createElement(SheetWrap, { title: 'Invite teammate', onClose, accent },
      React.createElement('div', { style: { padding: '4px 16px 28px', display: 'flex', flexDirection: 'column', gap: 14 } },
        React.createElement('div', { style: { fontFamily: SF, fontSize: 13, color: T.t2, lineHeight: 1.5 } },
          live ? 'They\u2019ll get a link that adds them straight into your workspace.' : 'No cloud connection — the person is recorded as invited on this device. Connect Cloud sync to send real invitations.'),
        React.createElement(Field, { label: 'Email', type: 'email', value: email, onChange: setEmail, placeholder: 'teammate@company.com', autoFocus: true }),
        ...
        React.createElement(PrimaryBtn, { accent, disabled: busy || !/\S+@\S+\.\S+/.test(email), onClick: send }, busy ? 'Creating…' : 'Create invitation'),
