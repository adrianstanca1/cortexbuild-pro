...
// DOCUMENTS
// ═══════════════════════════════════════════════════════════════════
const FILE_ICON_C = { pdf: T.red, dwg: T.cyan, xls: T.green, doc: T.blue, zip: T.amber };

function DocumentsScreen({ accent }) {
  ...
  return (
    <ScreenBg accent={accent}>
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
              ...
              {folders.map(f => {
                ...
                return (
                  <button key={f} onClick={() => setFolder(f)} style={{
                    ...
                    <div style={{ width: 32, height: 32, borderRadius: 8, background: `${accent}22`, color: accent, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {React.cloneElement(Ic.folder, { size: 17 })}
                    </div>
                    <div>
                      <div style={{ fontFamily: SF, fontSize: 13, color: T.t1, fontWeight: 600 }}>{f}</div>
                      <div style={{ fontFamily: SF, fontSize: 11, color: T.t2 }}>{count} {count === 1 ? 'file' : 'files'}</div>
                    </div>
                  </button>
                );
              })}
        ...
        <Section title={folder ? '' : 'Recent files'}>
          <GroupedList>
            {list.slice(0, folder ? 50 : 5).map((d, i, a) => {
              const c = FILE_ICON_C[d.type] || T.t3;
              const p = projects.find(p => p.id === d.projectId);
              return (
                <Row key={d.id} icon={Ic.doc} iconBg={c}
                  title={d.name}
            ...
            return (
              <div key={e.id} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                  <div>
                    <div style={{ fontFamily: SF, fontSize: 13, fontWeight: 600, color: T.t1 }}>{proj?.name}</div>
                    <div style={{ fontFamily: SF, fontSize: 11, color: T.t2, marginTop: 2 }}>{_formatRelDate(e.date)} · {new Date(e.date).toLocaleDateString('en-GB', { weekday: 'long' })}</div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontFamily: SFMono, fontSize: 12, color: T.t2 }}>
                    <span style={{ color: e.weather.cond === 'Sunny' ? T.amber : e.weather.cond === 'Rain' ? T.cyan : T.t2 }}>
                      {React.cloneElement(weatherIcon, { size: 14 })}
                    </span>
                    {e.weather.temp}°
                  </div>
                </div>
                <div style={{ fontFamily: SF, fontSize: 13, color: T.t1, lineHeight: 1.5 }}>{e.summary}</div>
                {e.notes && <div style={{ fontFamily: SF, fontSize: 12, color: T.t2, lineHeight: 1.5, marginTop: 6 }}>{e.notes}</div>}
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 10, fontFamily: SF, fontSize: 11, color: T.t3 }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>{React.cloneElement(Ic.team, { size: 13 })} {e.present}</span>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>{React.cloneElement(Ic.camera, { size: 13 })} {e.photos}</span>
                  {e.issues.length > 0 && <Pill c={T.amber} size="xs">{e.issues.length} issue{e.issues.length !== 1 && 's'}</Pill>}
                  <div style={{ flex: 1 }}/>
                </div>
              </div>
            );
...
function SnagsScreen({ accent }) {
  ...
  const toggle = (id, current) => Backend.db.snags.update(id, { status: current === 'open' ? 'fixed' : 'open' });
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
        ...
        <div style={{ padding: '4px 16px 14px' }}>
          <SegControl value={seg} onChange={setSeg} options={[
            ...
            { k: 'fixed', l: 'Fixed', n: fixed.length },
          ]}/>
        </div>
            ...
            return (
              <div key={s.id} onClick={() => toggle(s.id, s.status)} style={{
                ...
                <div style={{
                  ...
                  {s.status === 'fixed' && <span style={{ color: '#fff' }}>{React.cloneElement(Ic.check, { size: 13, sw: 3 })}</span>}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{
                    fontFamily: SF, fontSize: 13, color: T.t1, fontWeight: 500, lineHeight: 1.3,
...
// ═══════════════════════════════════════════════════════════════════
const CO_STATUS_C = { pending: T.amber, approved: T.green, rejected: T.red };
function ChangeOrdersScreen({ accent }) {
  ...
  const approve = (id) => window.cortexxNav('approval', {
    ...
    onApproved: async (allOK) => {
      if (allOK) {
        await Backend.db.changeOrders.update(id, { status: 'approved', approvedBy: 'Adrian' });
        toast('Variation approved', 'success');
      }
    },
  });
  const reject = (id) => Backend.db.changeOrders.update(id, { status: 'rejected' });
  return (
    <ScreenBg accent={accent}>
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
        <MobileHeader
          ...
          right={<HeaderBtn icon={Ic.plus} accent={accent} onClick={() => window.cortexxNav('addchange')}/>}
        />
          ...
          {cos.map(c => {
            ...
            return (
              <div key={c.id} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                  <div>
                    <Pill c={CO_STATUS_C[c.status]}>{c.status}</Pill>
                    <div style={{ fontFamily: SF, fontSize: 14, fontWeight: 600, color: T.t1, marginTop: 6 }}>{c.title}</div>
                    <div style={{ fontFamily: SFMono, fontSize: 11, color: T.t2, marginTop: 2 }}>{c.id} · {proj?.name?.split(' ').slice(0,2).join(' ')}</div>
                  </div>
                  <div style={{ fontFamily: SFMono, fontSize: 18, fontWeight: 700, color: T.t1 }}>+£{c.amount.toLocaleString()}</div>
                </div>
                ...
                {c.approvedBy && (
                  <div style={{ fontFamily: SF, fontSize: 10, color: T.t3, marginTop: 8 }}>
...
function InboxScreen({ accent, onAction }) {
  const notifs = useDB('notifications');
  const unread = notifs.filter(n => !n.read);
  const [seg, setSeg] = React.useState('all');
  const [portalMsgs, setPortalMsgs] = React.useState(() => {
    try { return JSON.parse(localStorage.getItem('cortexx_portal_messages') || '[]'); } catch (e) { return []; }
  });
  const unreadPortal = portalMsgs.filter(m => !m.read);
  const markPortalRead = (id) => {
    const next = portalMsgs.map(m => m.id === id ? { ...m, read: true } : m);
    setPortalMsgs(next);
    try { localStorage.setItem('cortexx_portal_messages', JSON.stringify(next)); } catch (e) {}
    if (window.CortexAudit) window.CortexAudit.log('You', 'read a client portal message', 'Inbox');
  };
  ...
  const markRead = (id) => Backend.db.notifications.update(id, { read: true });
  const markAllRead = () => unread.forEach(n => Backend.db.notifications.update(n.id, { read: true }));
    ...
    <ScreenBg accent={accent}>
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
        <MobileHeader
          ...
          ) : <HeaderBtn icon={Ic.cog} onClick={() => window.cortexxNav('profile')}/>}
        />
        <div style={{ padding: '4px 16px 14px' }}>
          <SegControl value={seg} onChange={setSeg} options={[
            { k: 'all', l: 'All', n: notifs.length },
            { k: 'unread', l: 'Unread', n: unread.length },
            { k: 'read', l: 'Read', n: notifs.length - unread.length },
          ]}/>
        </div>
        <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
          {portalMsgs.length > 0 && seg !== 'read' && (
            <div style={{ marginBottom: 4 }}>
              <div style={{ fontFamily: SF, fontSize: 11, fontWeight: 700, color: T.purple, textTransform: 'uppercase', letterSpacing: 0.6, margin: '4px 2px 8px', display: 'flex', alignItems: 'center', gap: 6 }}>
                {React.cloneElement(Ic.team, { size: 12 })} Client portal messages{unreadPortal.length > 0 ? ` · ${unreadPortal.length} new` : ''}
              </div>
                ...
                {portalMsgs.filter(m => seg === 'unread' ? !m.read : true).map(m => (
                  <div key={m.id} onClick={() => { markPortalRead(m.id); if (window.cortexxNav) window.cortexxNav('clientmsgs'); }} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
                      <Avatar name={m.client} size={26} c={T.purple}/>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontFamily: SF, fontSize: 13, fontWeight: 600, color: T.t1 }}>{m.client}</div>
                        <div style={{ fontFamily: SF, fontSize: 10, color: T.t3 }}>{m.project} · {timeAgo(m.when)}</div>
                      </div>
                      {m.replied ? <Pill c={T.green} size="xs">REPLIED</Pill> : !m.read && <span style={{ width: 8, height: 8, borderRadius: 4, background: T.purple, flexShrink: 0 }}/>}
                    </div>
                    <div style={{ fontFamily: SF, fontSize: 13, color: T.t2, lineHeight: 1.45, paddingLeft: 34 }}>{m.text}</div>
                    <div style={{ fontFamily: SF, fontSize: 11, fontWeight: 700, color: T.purple, paddingLeft: 34, marginTop: 6 }}>Reply →</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {list.map(n => {
            ...
            return (
              <div key={n.id} onClick={() => {
                ...
                {!n.read && <div style={{ position: 'absolute', top: 14, right: 12, width: 8, height: 8, borderRadius: 4, background: n.color }}/>}
                <div style={{ width: 36, height: 36, borderRadius: 9, background: `${n.color}22`, color: n.color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  {React.cloneElement(Icon, { size: 17 })}
                </div>
                <div style={{ flex: 1, minWidth: 0, paddingRight: !n.read ? 14 : 0 }}>
                  <div style={{ fontFamily: SF, fontSize: 13, fontWeight: 600, color: T.t1, lineHeight: 1.3 }}>{n.t}</div>
                  <div style={{ fontFamily: SF, fontSize: 11, color: T.t2, marginTop: 2 }}>{n.sub}</div>
                  <div style={{ fontFamily: SFMono, fontSize: 9, color: T.t3, marginTop: 4 }}>
                    {new Date(n.when).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })} · {_formatRelDate(n.when.slice(0, 10))}
                  </div>
                </div>
              </div>
            );
...
// ═══════════════════════════════════════════════════════════════════
function OnboardingSheet({ onClose, accent }) {
    ...
    return (
      <Sheet onClose={onClose} fullscreen>
        <div style={{ flex: 1, padding: '40px 28px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ fontFamily: SF, fontSize: 32, fontWeight: 700, color: T.t1, letterSpacing: -1, lineHeight: 1.1, marginTop: 24 }}>
            Welcome to CortexBuild Pro.
          </div>
          <div style={{ fontFamily: SF, fontSize: 15, color: T.t2, lineHeight: 1.5, marginTop: 12 }}>
            The construction manager that thinks alongside you. Quotes, jobs, money, safety — all in one place, with a UK-aware AI ops lead built in.
          </div>
          <div style={{ marginTop: 28, display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[
              { i: Ic.calc,   l: 'AI estimates',        s: 'Describe a job, get realistic line items in seconds' },
              { i: Ic.spark,  l: 'Daily briefing',      s: 'Cortex tells you what to focus on each morning' },
              { i: Ic.receipt,l: 'Auto-categorised',    s: 'Snap receipts, AI files them to the right job' },
              { i: Ic.shield, l: 'CIS & H&S aware',     s: 'Built for UK SMB contractors from day one' },
            ].map((f, i) => (
              <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                ...
                <div>
                  <div style={{ fontFamily: SF, fontSize: 14, fontWeight: 600, color: T.t1 }}>{f.l}</div>
                  <div style={{ fontFamily: SF, fontSize: 12, color: T.t2, marginTop: 1 }}>{f.s}</div>
                </div>
              </div>
            ))}
          </div>
    ...
    return (
      <Sheet onClose={onClose} fullscreen>
        <div style={{ flex: 1, padding: '60px 28px', display: 'flex', flexDirection: 'column' }}>
          <div style={{ fontFamily: SF, fontSize: 11, color: accent, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1.2 }}>Step 1 of 2</div>
          <div style={{ fontFamily: SF, fontSize: 28, fontWeight: 600, color: T.t1, letterSpacing: -0.8, marginTop: 8, lineHeight: 1.15 }}>
            What should we call you?
          </div>
          <div style={{ fontFamily: SF, fontSize: 14, color: T.t2, marginTop: 8, lineHeight: 1.5 }}>
            Cortex uses your name to personalise briefings.
          </div>
          <input
  ...
  return (
    <Sheet onClose={onClose} fullscreen>
      <div style={{ flex: 1, padding: '60px 28px', display: 'flex', flexDirection: 'column' }}>
        <div style={{ fontFamily: SF, fontSize: 11, color: accent, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1.2 }}>Step 2 of 2</div>
        <div style={{ fontFamily: SF, fontSize: 28, fontWeight: 600, color: T.t1, letterSpacing: -0.8, marginTop: 8, lineHeight: 1.15 }}>
          Tell Cortex about your business.
        </div>
        <div style={{ fontFamily: SF, fontSize: 14, color: T.t2, marginTop: 8, lineHeight: 1.5 }}>
          Even a sentence helps — we'll seed sensible defaults you can edit.
        </div>
        <textarea
          ...
          style={{
            marginTop: 24, background: T.bg2, border: `0.5px solid ${T.hairMid}`,
            borderRadius: 12, padding: '14px 16px',
            color: T.t1, fontFamily: SF, fontSize: 15, outline: 'none',
...
Object.assign(window, {
  DocumentsScreen, DiaryScreen, SnagsScreen, ChangeOrdersScreen,
  InboxScreen, OnboardingSheet,
});
