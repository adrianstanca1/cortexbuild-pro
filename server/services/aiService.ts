...
// Only initialize Gemini if we're using it as the provider
let ai: GoogleGenAI | null = null;

if (AI_PROVIDER === 'gemini' || AI_PROVIDER === 'abacus') {
    if (!apiKey) {
        logger.error('GEMINI_API_KEY is not configured but AI_PROVIDER is set to gemini/abacus!');
    } else {
        ai = new GoogleGenAI({ apiKey });
        logger.info('Gemini AI initialized');
    }
} else {
    logger.info(`AI Provider set to: ${AI_PROVIDER}. Gemini API not initialized.`);
}
...
export const analyzeProjectDocument = async (fileBuffer: Buffer, mimeType: string, projectContext: any = {}) => {
    try {
        if (!ai) throw new Error("Gemini AI not initialized. Set AI_PROVIDER='gemini' and GEMINI_API_KEY to use this feature.");
