...
export const ProjectActionModals: React.FC<ProjectActionModalsProps> = ({ type, projectId, onClose }) => {
    ...
    const inputClass =
        'w-full p-3 bg-white border border-zinc-200 rounded-xl text-sm focus:ring-2 focus:ring-[#0f5c82] focus:border-[#0f5c82] outline-none transition-all text-zinc-900 placeholder:text-zinc-400';
    ...
    const handleSubmitPhoto = async () => {
        ...
        if (photoFile) {
            try {
                const { publicUrl } = await import('@/services/storageService').then((m) =>
                    m.storageService.upload(photoFile, 'documents', `projects/${projectId}/photos`)
                );
            ...
            <Modal isOpen={!!type} onClose={onClose} title="New Request for Information" size="md">
                <div className="flex items-center gap-3 mb-4 text-[#0f5c82] bg-blue-50 p-3 rounded-lg border border-blue-100">
                    ...
                    <span className="text-sm font-medium text-blue-900">
                        Request information or clarification from the project team.
                    </span>
                </div>
                <form onSubmit={handleSubmitRFI}>
                    <InputGroup label="Subject">
                        <input
                            ...
                            placeholder="e.g. Clarification on steel spec"
                    ...
                    <InputGroup label="Question / Issue">
                        <textarea
                            ...
                            onChange={(e) => setRfiData({ ...rfiData, question: e.target.value })}
                            required
                        />
                    </InputGroup>
                    <div className="grid grid-cols-2 gap-4">
                        <InputGroup label="Assigned To">
                            <input
                                ...
                                onChange={(e) => setRfiData({ ...rfiData, assignedTo: e.target.value })}
                            />
                        </InputGroup>
                        <InputGroup label="Due Date">
                            <input
                                ...
                                onChange={(e) => setRfiData({ ...rfiData, dueDate: e.target.value })}
                            />
                        </InputGroup>
                    </div>
                    <button
                        ...
                        className="w-full py-3 bg-[#0f5c82] text-white rounded-xl font-bold hover:bg-[#0c4a6e] transition-all shadow-md mt-2"
                    >
                ...
                <form onSubmit={handleSubmitPunch}>
                    <InputGroup label="Item Title">
                        <input
                            type="text"
                            className={inputClass}
                            placeholder="e.g. Paint scratch"
                            value={punchData.title}
                            onChange={(e) => setPunchData({ ...punchData, title: e.target.value })}
                            required
                        />
                    </InputGroup>
                    <div className="grid grid-cols-2 gap-4">
                        <InputGroup label="Location">
                            <input
                                ...
                                value={punchData.location}
                                onChange={(e) => setPunchData({ ...punchData, location: e.target.value })}
                                required
                            />
                        </InputGroup>
                        <InputGroup label="Priority">
                            <select
                                className={inputClass}
                                value={punchData.priority}
                                onChange={(e) => setPunchData({ ...punchData, priority: e.target.value as any })}
                            >
                    ...
                    <InputGroup label="Description">
                        <textarea
                            ...
                            placeholder="Details..."
                            value={punchData.description}
                            onChange={(e) => setPunchData({ ...punchData, description: e.target.value })}
                        />
                    </InputGroup>
                    <button
                        type="submit"
                        disabled={isSubmitting}
                        className="w-full py-3 bg-orange-600 text-white rounded-xl font-bold hover:bg-orange-700 transition-all shadow-md mt-2"
                    >
            ...
            <Modal isOpen={!!type} onClose={onClose} title="Daily Site Log" size="md">
                <div className="flex items-center gap-3 mb-4 text-purple-600 bg-purple-50 p-3 rounded-lg border border-purple-100">
                    ...
                    <span className="text-sm font-medium text-purple-900">
                        Record daily site progress and conditions.
                    </span>
                </div>
                <form onSubmit={handleSubmitLog}>
                    <div className="flex justify-end mb-2">
                        <button
                            type="button"
                            onClick={generateLogWithAI}
                            disabled={isGeneratingLog}
                            className="text-xs flex items-center gap-1 text-purple-600 hover:bg-purple-50 px-2 py-1 rounded font-bold transition-colors"
                        >
                            {isGeneratingLog ? <Loader2 size={12} className="animate-spin" /> : <ScanLine size={12} />}{' '}
                            Auto-Fill with AI
                        </button>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <InputGroup label="Crew Count">
                            <input
                                type="number"
                                className={inputClass}
                                value={logData.crewCount}
                                onChange={(e) => setLogData({ ...logData, crewCount: parseInt(e.target.value) })}
                            />
                        </InputGroup>
                    </div>
                    <InputGroup label="Weather Conditions">
                        <input
                            ...
                            onChange={(e) => setLogData({ ...logData, weather: e.target.value })}
                        />
                    </InputGroup>
                    <InputGroup label="Work Performed">
                        <textarea
                            ...
                            onChange={(e) => setLogData({ ...logData, workPerformed: e.target.value })}
                            required
                        />
                    </InputGroup>
                ...
                <form onSubmit={handleSubmitDaywork} className="space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-sm">
                        <h4 className="text-sm font-bold text-zinc-800 mb-4 flex items-center gap-2">
                            <Clipboard size={16} /> Sheet Details
                        </h4>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => setDayworkData({ ...dayworkData, date: e.target.value })}
                                    required
                                />
                            </div>
                            <div className="md:col-span-2">
                                ...
                                <textarea
                                    className={`${inputClass} h-[46px] resize-none py-2`}
                                    placeholder="Summary of work carried out..."
                                    value={dayworkData.description}
                                    onChange={(e) => setDayworkData({ ...dayworkData, description: e.target.value })}
                                    required
                                />
                            </div>
                        </div>
                    ...
                    <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-sm">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button
                                type="button"
                                onClick={addLaborRow}
                                className="text-[#0f5c82] text-xs font-bold hover:bg-blue-50 px-2 py-1 rounded transition-colors"
                            >
                                + Add Worker
                            </button>
                        </div>
                        <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                                {laborRows.map((row, i) => (
                                    <div
                                        key={i}
                                        className="grid grid-cols-12 gap-2 items-center bg-zinc-50 p-2 rounded-lg border border-zinc-100"
                                    >
                                        <div className="col-span-4">
                                            <input
                                                type="text"
                                                placeholder="Worker Name"
                                                className="w-full p-2 text-sm border border-zinc-200 rounded bg-white focus:border-[#0f5c82] outline-none"
                                                value={row.name}
                                                onChange={(e) => updateLaborRow(i, 'name', e.target.value)}
                                            />
                                        </div>
                                        <div className="col-span-3">
                                            <input
                                                ...
                                                className="w-full p-2 text-sm border border-zinc-200 rounded bg-white focus:border-[#0f5c82] outline-none"
                                                value={row.trade}
                                                onChange={(e) => updateLaborRow(i, 'trade', e.target.value)}
                                            />
                                        </div>
                                        <div className="col-span-2">
                                            <input
                                                ...
                                                className="w-full p-2 text-sm border border-zinc-200 rounded bg-white focus:border-[#0f5c82] outline-none"
                                                value={row.hours}
                                                onChange={(e) => updateLaborRow(i, 'hours', parseFloat(e.target.value))}
                                            />
                                        </div>
                                        <div className="col-span-2">
                                            <input
                                                ...
                                                className="w-full p-2 text-sm border border-zinc-200 rounded bg-white focus:border-[#0f5c82] outline-none"
                                                value={row.rate}
                                                onChange={(e) => updateLaborRow(i, 'rate', parseFloat(e.target.value))}
                                            />
                                        </div>
                            ...
                            <div className="flex justify-end mt-2 px-2">
                                <div className="text-sm font-bold text-zinc-700">
                                    Total Labour:{' '}
                                    <span className="text-[#0f5c82]">
                                        £
                                        {laborRows
                                            .reduce((acc, row) => acc + row.hours * (row.rate || 0), 0)
                                            .toFixed(2)}
                                    </span>
                                </div>
                            </div>
                    ...
                    <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-sm">
                        <div className="flex justify-between items-center mb-4">
                            <h4 className="text-sm font-bold text-zinc-800 flex items-center gap-2">
                                <Hammer size={16} /> Materials & Plant
                            </h4>
                            <button
                                type="button"
                        ...
                        <div className="space-y-2">
                                    ...
                                    >
                                        ...
                                        <div className="col-span-2">
                                            <input
                                                ...
                                                value={row.quantity}
                                                onChange={(e) =>
                                                    updateMaterialRow(i, 'quantity', parseFloat(e.target.value))
                                                }
                                            />
                                        </div>
                                        <div className="col-span-2">
                                        ...
                                        <div className="col-span-2">
                                            <input
                                                ...
                                                value={row.cost}
                                                onChange={(e) =>
                                                    updateMaterialRow(i, 'cost', parseFloat(e.target.value))
                                                }
                                            />
                                        </div>
                                        <div className="col-span-1 flex justify-center">
                            ...
                            <div className="flex justify-end mt-2 px-2">
                                <div className="text-sm font-bold text-zinc-700">
                                    Total Materials:{' '}
                                    <span className="text-[#0f5c82]">
                                        £{materialRows.reduce((acc, row) => acc + (row.cost || 0), 0).toFixed(2)}
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-sm">
                        <h4 className="text-sm font-bold text-zinc-800 mb-4 flex items-center gap-2">
                            <Paperclip size={16} /> Documentation
                        </h4>
                        <div className="flex flex-wrap gap-4">
                            {dayworkAttachments.map((file, i) => (
                                <div
                                    key={i}
                                    className="relative group w-24 h-24 bg-zinc-50 border border-zinc-200 rounded-xl overflow-hidden flex flex-col items-center justify-center shadow-sm hover:shadow-md transition-all"
                                >
                                    ...
                                    <button
                                        ...
                                        className="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity transform scale-75 hover:scale-100"
                                    >
                                        <X size={12} />
                                    </button>
                                    {!file.type.startsWith('image/') && (
                                        <span className="text-[9px] text-zinc-500 mt-2 px-1 truncate w-full text-center">
                                            {file.name}
                                        </span>
                                    )}
                                </div>
                            ...
                            <input
                                ...
                                multiple
                                accept="image/*,.pdf"
                                onChange={handleDayworkAttachment}
                            />
                        </div>
                    </div>
                    ...
                    <div className="bg-zinc-900 text-white p-4 rounded-xl flex justify-between items-center shadow-lg">
                        <div>
                            ...
                            <div className="text-2xl font-bold">
                                £
                                {(
                                    laborRows.reduce((acc, row) => acc + row.hours * (row.rate || 0), 0) +
                                    materialRows.reduce((acc, row) => acc + (row.cost || 0), 0)
                                ).toFixed(2)}
                            </div>
                        </div>
                        <div className="flex gap-3">
                            <button
                                type="button"
                                onClick={onClose}
                                className="px-6 py-3 rounded-xl text-sm font-bold text-zinc-300 hover:bg-white/10 transition-colors"
                            >
                                Cancel
                            </button>
                            <button
                                type="submit"
                                disabled={isSubmitting}
                                className="px-8 py-3 bg-white text-zinc-900 rounded-xl text-sm font-bold hover:bg-zinc-200 transition-colors shadow-md flex items-center gap-2"
                            >
                                {isSubmitting ? (
                ...
                <div className="space-y-6">
                    {!isCameraOpen && !photoPreview && (
                        <div className="grid grid-cols-2 gap-4">
                            <button
                                onClick={startCamera}
                                className="flex flex-col items-center justify-center p-8 bg-zinc-50 border-2 border-dashed border-zinc-300 rounded-2xl hover:bg-blue-50 hover:border-[#0f5c82] transition-all group"
                            >
                                <Camera size={32} className="text-zinc-400 group-hover:text-[#0f5c82] mb-2" />
                                <span className="text-sm font-bold text-zinc-600 group-hover:text-[#0f5c82]">
                                    Take Photo
                                </span>
                            </button>
                            <button
                    ...
                    {isCameraOpen && (
                        <div className="relative bg-black rounded-2xl overflow-hidden aspect-[4/3]">
                            ...
                            <button
                                ...
                                className="absolute bottom-4 left-1/2 -translate-x-1/2 w-16 h-16 bg-white rounded-full border-4 border-zinc-300 shadow-lg active:scale-95 transition-transform"
                            />
                            <button
                        ...
                        <div className="space-y-4">
                            <div className="relative rounded-2xl overflow-hidden border border-zinc-200 aspect-video bg-zinc-100">
                                ...
                                <button
                                    ...
                                    className="absolute top-2 right-2 p-1 bg-white/80 rounded-full hover:bg-red-100 hover:text-red-500"
                                >
                                    <X size={16} />
                                </button>
                            </div>
