...
export enum Page {
  ...
  ACCOUNTING_HUB = 'ACCOUNTING_HUB',
  AI_COMMAND_CENTRE = 'AI_COMMAND_CENTRE',
  MAP_VIEW = 'MAP_VIEW',
...
export interface Company {
  ...
  name: string;
  slug: string;
  plan: 'trial' | 'starter' | 'pro' | 'enterprise' | 'Enterprise' | 'Business' | 'Starter' | 'Custom' | 'Free' | 'Pro' | 'trialing';
  status: 'trial' | 'active' | 'suspended' | 'expired' | 'cancelled' | 'Active' | 'Suspended' | 'Trial' | 'Inactive';
  industry?: string;
  region?: string;
  users?: number;
  projects?: number;
  ...
  joinedDate?: string; // Legacy support
  // Trial fields
...
export interface Project {
  ...
  tags?: string[]; // Project tags/labels
  ...
  activeCollaborators?: string[]; // Array of user IDs currently working
  recentComments?: Array<{ userId: string; userName: string; text: string; timestamp: string }>; // Recent comments
  customFields?: Record<string, any>; // Custom data
}
...
export interface StorageUsage {
  fileStorage: StorageMetrics;
  databaseStorage: StorageMetrics;
  totalUsage: {
    used: number;
    quota: number;
    percentage: number;
