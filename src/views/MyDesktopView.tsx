...
const MyDesktopView: React.FC<MyDesktopViewProps> = ({ installedApps, setPage }) => {
    ...
    const [fileSystem, setFileSystem] = useState<Record<string, FileNode>>({
        ...
        documents: {
            ...
            type: 'folder',
            parentId: 'admin',
            children: ['project_alpha', 'budget_2025'],
            created: Date.now()
        },
        desktop: {
            id: 'desktop',
        ...
        note1: {
            ...
            type: 'file',
            content: '- Review safety protocols\n- Discuss Q4 roadmap\n- Team lunch at 12:30',
            parentId: 'desktop',
            created: Date.now()
        },
        boot_log: {
        ...
        readme: {
            ...
            name: 'README.md',
            type: 'file',
            content: '# BuildOS v2.5\nWelcome to the next generation construction OS.',
            parentId: 'docs',
            created: Date.now()
        }
    });
    ...
    const [widgetDragState, setWidgetDragState] = useState<{
        id: string;
        startX: number;
        startY: number;
        initX: number;
        ...
        setDesktopWidgets((prev) =>
            prev.map((w) =>
                w.id === widgetDragState.id
                    ? {
                          ...w,
                          position: { x: Math.max(0, widget.position.x + dx), y: Math.max(0, widget.position.y + dy) },
                          lastModified: Date.now()
                      }
                    : w
            )
    ...
    const toggleWidgetPin = (widgetId: string) => {
        setDesktopWidgets((prev) => prev.map((w) => (w.id === widgetId ? { ...w, isPinned: !w.isPinned } : w)));
    };
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        if (dragRef.current) {
            ...
            setWindows((prev) =>
                prev.map((w) => (w.id === id ? { ...w, position: { x: initX + deltaX, y: initY + deltaY } } : w))
            );
        }
    };
    ...
    const openApp = (
        ...
        if (existing) {
            setWindows((prev) =>
                prev.map((w) => (w.id === existingId ? { ...w, isMinimized: false, isOpen: true } : w))
            );
                ...
                <div className="flex-1 flex overflow-hidden">
                    ...
                    <div className="w-48 bg-white border-r border-zinc-200 p-2 flex-shrink-0 hidden sm:block">
                        ...
                        <button
                            ...
                            className="w-full text-left flex items-center gap-2 px-2 py-1.5 hover:bg-zinc-100 rounded text-sm text-zinc-600"
                        >
    ...
    const TerminalApp = () => {
        ...
        const execute = (input: string) => {
            ...
            switch (command) {
                ...
                case 'rm':
                    if (args[1]) {
                        const targetToDelete = (fileSystem[cwdId].children || []).find(
                ...
                case 'cat': {
                    if (args[1]) {
                        const targetFile = (fileSystem[cwdId].children || []).find(
                            (id) => fileSystem[id].name === args[1]
                        );
        ...
        return (
            <div
                className="h-full bg-black text-green-400 font-mono text-sm p-2 overflow-y-auto flex flex-col"
        ...
        return (
            <div className="flex flex-col h-full bg-white text-zinc-900">
                <div className="h-8 border-b border-zinc-200 flex items-center gap-2 px-2 bg-zinc-50 select-none">
                    <button
                        onClick={handleSave}
                        className="flex items-center gap-1 px-2 py-1 hover:bg-zinc-200 rounded text-xs"
                    >
                        <Save size={12} /> Save
                    </button>
        ...
        return (
            <div className="h-full bg-zinc-900 p-2 grid grid-cols-2 gap-2 overflow-y-auto">
                {[1, 2, 3, 4].map((id) => (
                    <div
                        key={id}
                        className="relative bg-black rounded-lg overflow-hidden group aspect-video border border-zinc-800"
                    >
                ...
                <div>
                    ...
                    <div className="h-24 border border-zinc-800 bg-zinc-900 rounded relative overflow-hidden">
                        <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none">
                            <path
                                d="M0,50 Q50,20 100,60 T200,40 T300,80"
                                fill="none"
                                stroke="#4ade80"
                                strokeWidth="1.5"
                            />
                            <path
                                d="M0,60 Q50,90 100,40 T200,70 T300,30"
                                fill="none"
                                stroke="#22d3ee"
                                strokeWidth="1.5"
                                strokeDasharray="4 4"
    ...
    const desktopApps = useMemo(
        () => [
            ...
            { id: 'browser', title: 'Web Browser', icon: Globe, content: <BrowserApp />, width: 1000, height: 700 },
            {
                ...
                title: 'Marketplace',
                icon: Grid,
                content: <MarketplaceView setPage={setPage} toggleInstall={() => {}} />,
                width: 900,
                height: 600
            },
            ...
            },
            {
                ...
                title: 'AI Chat',
                icon: MessageSquare,
                content: <ChatView setPage={() => {}} />,
                width: 500,
                height: 700
            }
        ],
        [fileSystem, installedApps]
    ); // Re-create if FS changes for new Notepad instances
        ...
        >
            ...
            {contextMenu && (
                ...
                >
                    <button
                        onClick={() => {
                            createFile('desktop', `New Folder ${Date.now()}`, 'folder');
                            closeContextMenu();
                        }}
                        className="w-full text-left px-4 py-2 text-sm text-zinc-700 hover:bg-[#0f5c82] hover:text-white flex items-center gap-2"
                    >
                        <Folder size={14} /> New Folder
                    </button>
                    <button
                        onClick={() => {
                            openApp('notepad', 'Notepad', Type, <NotepadApp />);
                            closeContextMenu();
                        }}
                        className="w-full text-left px-4 py-2 text-sm text-zinc-700 hover:bg-[#0f5c82] hover:text-white flex items-center gap-2"
                    >
            ...
            {windows.map((win) => (
                ...
                >
                    {/* Title Bar */}
                    <div
                        className="h-10 bg-zinc-100 border-b border-zinc-200 flex items-center justify-between px-3 flex-shrink-0 select-none"
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className="p-1.5 hover:bg-red-100 hover:text-red-600 rounded text-zinc-500"
                            >
                                <X size={12} />
                            </button>
                        </div>
                ...
                <div className="absolute bottom-14 left-4 w-80 bg-white/90 backdrop-blur-xl border border-zinc-200/50 rounded-xl shadow-2xl p-4 z-[2000] animate-in slide-in-from-bottom-5">
                    ...
                    <div className="grid grid-cols-4 gap-2 mb-4 max-h-64 overflow-y-auto custom-scrollbar">
                        {desktopApps.map((app) => (
                            <button
                                key={app.id}
                                onClick={() => openApp(app.id, app.title, app.icon, app.content, app.width, app.height)}
                                className="flex flex-col items-center gap-1 p-2 hover:bg-zinc-100 rounded-lg transition-colors group"
                            >
                                <div className="p-2 bg-[#0f5c82]/10 rounded-lg text-[#0f5c82] group-hover:bg-[#0f5c82] group-hover:text-white transition-colors">
                                    <app.icon size={20} />
                                </div>
                    ...
                    <div className="border-t border-zinc-200 pt-3 flex justify-between items-center px-2">
                        ...
                        <button
                            ...
                            className="p-2 hover:bg-zinc-100 rounded text-zinc-500 hover:text-red-600"
                        >
                            <Power size={18} />
                        </button>
                    </div>
                ...
                <div className="absolute bottom-14 right-4 w-80 bg-white/90 backdrop-blur-xl border border-zinc-200/50 rounded-xl shadow-2xl p-6 z-[2000] animate-in slide-in-from-bottom-5">
                    ...
                    <div className="grid grid-cols-7 gap-2 text-center text-sm">
                        {[...Array(30)].map((_, i) => (
                            <button
                                key={i}
                                className={`w-8 h-8 rounded-full flex items-center justify-center hover:bg-zinc-100 ${i + 1 === currentTime.getDate() ? 'bg-[#0f5c82] text-white hover:bg-[#0f5c82]' : ''}`}
                            >
                                {i + 1}
                            </button>
                        ))}
            ...
            <div className="h-12 bg-white/80 backdrop-blur-md border-t border-white/20 flex items-center justify-between px-4 z-[2000] relative">
                ...
                <div className="flex items-center gap-3 text-zinc-600 pl-4 border-l border-zinc-300/50 h-full">
                    <button
                        ...
                        className={`hidden sm:flex items-center gap-2 px-2 py-1 rounded hover:bg-white/50 ${showQuickSettings ? 'bg-white/50' : ''}`}
                    >
                        <Wifi size={14} />
                        <Volume2 size={14} />
                        <Battery size={14} />
                    ...
                    <button
                        ...
                        className="w-1.5 h-8 border-l border-zinc-300 ml-1 hover:bg-zinc-200 rounded-full"
                        title="Show Desktop"
                    ></button>
                </div>
            </div>
