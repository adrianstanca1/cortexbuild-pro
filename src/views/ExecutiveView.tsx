...
const ExecutiveView: React.FC = () => {
  ...
  const stats = useMemo(() => {
    const totalPortfolio = projects.reduce((sum, p) => sum + p.budget, 0);
    const activeProjects = projects.filter(p => p.status === 'Active').length;
    const avgSafety = 98.2; // Static for now as SafetyContext isn't available
    const utilization = 85; // Static for now
    return { totalPortfolio, activeProjects, avgSafety, utilization };
  }, [projects]);

  const riskDistribution = useMemo(() => {
    const high = projects.filter(p => p.health === 'Critical').length;
    const medium = projects.filter(p => p.health === 'At Risk').length;
    const low = projects.filter(p => p.health === 'Good' || p.status === 'Completed').length;
    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white border border-zinc-200 rounded-xl p-6 shadow-sm">
          ...
          <div className="text-3xl font-bold text-zinc-900 mb-1">£{(stats.totalPortfolio / 1000000).toFixed(1)}M</div>
        ...
        <div className="bg-white border border-zinc-200 rounded-xl p-6 shadow-sm">
          ...
          <div className="text-3xl font-bold text-zinc-900 mb-1">{stats.avgSafety}%</div>
        ...
        <div className="bg-white border border-zinc-200 rounded-xl p-6 shadow-sm">
          ...
          <div className="text-3xl font-bold text-zinc-900 mb-1">{stats.utilization}%</div>
        ...
        <div className="bg-white border border-zinc-200 rounded-xl p-6 shadow-sm">
          ...
          <div className="text-3xl font-bold text-zinc-900 mb-1">{stats.activeProjects}</div>
          ...
          <div className="flex-1 flex items-center justify-center relative">
            ...
            <div className="w-48 h-48 rounded-full border-[24px] border-[#e4e4e7] relative rotate-45">
              ...
              <div className="absolute inset-0 flex items-center justify-center flex-col">
                <span className="text-3xl font-bold text-zinc-800">{Math.round((riskDistribution.high / projects.length) * 100) || 0}%</span>
