                    ...
                    <div className="flex-1 bg-black relative flex flex-col">
                        ...
                        <div className="h-16 bg-zinc-900 border-t border-zinc-800 p-3 flex items-center gap-4 shrink-0">
                            ...
                            <form onSubmit={handleAgentQuery} className="flex-1 relative">
                                ...
                                <button type="submit" disabled={!agentQuery.trim() || isAgentThinking} aria-label="Send Query" title="Send Query" className="absolute right-2 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-white disabled:opacity-30"><Send size={14} /></button>
                            </form>
                            {isCameraActive && <button onClick={stopCamera} aria-label="Stop Camera" title="Stop Camera" className="p-2 bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30 transition-colors"><StopCircle size={18} /></button>}
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-1.5 rounded-2xl border border-zinc-200 shadow-sm mb-6">
                        <form onSubmit={handleGenerateImage} className="flex flex-col md:flex-row items-center gap-2 p-2">
                            ...
                            <div className="relative h-full">
                                ...
                                <select value={imgAspectRatio} onChange={(e) => setImgAspectRatio(e.target.value)} aria-label="Image Aspect Ratio" className="w-full h-full bg-zinc-50 border-none rounded-lg text-sm text-zinc-700 py-3 pl-10 pr-10 focus:ring-0 cursor-pointer appearance-none font-medium">
                            ...
                            {currentImage ? (
                                <div className="relative w-full h-full flex items-center justify-center bg-black/5">
                                    ...
                                    <div className="absolute top-4 right-4 flex gap-2">
                                        <button onClick={() => window.open(currentImage.url, '_blank')} aria-label="Download Image" title="Download Image" className="p-2 bg-white rounded-lg shadow-sm hover:bg-zinc-50 border border-zinc-200"><Download size={18} /></button>
                                        <button aria-label="Share Image" title="Share Image" className="p-2 bg-white rounded-lg shadow-sm hover:bg-zinc-50 border border-zinc-200"><Share2 size={18} /></button>
                                    </div>
                                </div>
                            ) : (
                            ...
                            <div className="flex-1 overflow-y-auto p-2 space-y-2">
                                {history.map((h, i) => (
                                    <div key={h.id || i} onClick={() => setCurrentImage(h)} className={`p-2 rounded-lg border cursor-pointer hover:bg-zinc-50 flex gap-3 ${currentImage?.id === h.id ? 'border-[#0f5c82] bg-blue-50/50' : 'border-zinc-100'}`}>
                                        <img src={h.url} alt={`Generated: ${h.prompt}`} className="w-16 h-16 object-cover rounded-md bg-zinc-100 shrink-0" />
                        ...
                        <div className="relative z-10 grid lg:grid-cols-2 gap-12">
                            <div>
                                <div className="mb-6 p-4 bg-purple-800/50 border border-purple-400/30 rounded-xl backdrop-blur-sm">
                                    <h3 className="font-bold flex items-center gap-2 mb-2"><Sparkles size={16} className="text-yellow-400" /> Veo AI (Alpha Release)</h3>
                                    <p className="text-sm text-purple-100 leading-relaxed">
                                        Generative Video for construction. Describe your site scene or safety simulation to visualize potential outcomes.
                                    </p>
                                </div>
                                ...
                                <form onSubmit={handleGenerateVideo} className="space-y-4">
                                    <textarea
                                        ...
                                        placeholder="Describe the construction site scene (e.g., Drone flythrough of a high-rise foundation excavation at sunset)..."
                                        className="w-full bg-white/10 border border-white/20 rounded-xl p-4 text-white placeholder-white/50 focus:ring-2 focus:ring-purple-400 outline-none h-32 resize-none transition-all"
                                    />
                                    <div className="flex gap-4">
                                        <button
                                            type="button"
                                            onClick={() => setVidAspectRatio('16:9')}
                                            className={`flex-1 flex items-center justify-center gap-2 bg-white/10 px-4 py-3 rounded-xl border transition-all ${vidAspectRatio === '16:9' ? 'border-white bg-white/20' : 'border-white/10 hover:bg-white/5'}`}
                                        >
                                            <span>16:9 Landscape</span>
                                        </button>
                                        <button
                                            type="button"
                                            onClick={() => setVidAspectRatio('9:16')}
                                            className={`flex-1 flex items-center justify-center gap-2 bg-white/10 px-4 py-3 rounded-xl border transition-all ${vidAspectRatio === '9:16' ? 'border-white bg-white/20' : 'border-white/10 hover:bg-white/5'}`}
                                        >
                                    ...
                                    <button
                                        type="submit"
                                        disabled={isVidGenerating || !vidPrompt.trim()}
                                        className="w-full py-4 bg-white text-purple-900 font-black rounded-xl hover:bg-purple-100 disabled:opacity-50 transition-all flex items-center justify-center gap-3 shadow-xl"
                                    >
                                        {isVidGenerating ? <Loader2 size={24} className="animate-spin" /> : <><Zap size={20} className="fill-purple-900" /> Generate Veo Video</>}
                                    </button>
                                </form>
                            </div>

                            <div className="flex flex-col">
                                <div className="flex-1 bg-black/40 border border-white/10 rounded-2xl flex items-center justify-center overflow-hidden min-h-[300px] relative group">
                                    {currentVideoUrl ? (
                                        <video
                                            src={currentVideoUrl}
                                            controls
                                        ...
                                        <div className="text-center text-purple-200/50">
                                            {isVidGenerating ? (
                                                <div className="flex flex-col items-center gap-4">
                                                    ...
                                                    <p className="font-bold">Gemini is rendering your scene...</p>
                                                </div>
                                            ) : (
                                                <div className="flex flex-col items-center gap-4">
                                ...
                                {currentVideoUrl && (
                                    <div className="mt-4 flex gap-3">
                                        <button onClick={() => window.open(currentVideoUrl)} className="flex-1 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-sm font-bold border border-white/10 transition-all">Download Result</button>
                                        <button onClick={() => showNotification("Video shared to project library")} className="flex-1 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-sm font-bold border border-white/10 transition-all">Share to Project</button>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {mode === 'INSPECT' && (
                <div className="flex-1 flex overflow-hidden">
                    <div className="flex-1 bg-zinc-100 p-6 flex flex-col justify-center items-center border-r border-zinc-200 relative">
                        ...
                        {analyzeMedia && <button onClick={() => { setAnalyzeMedia(null); setAnalysisResult(null); }} aria-label="Clear Media" title="Clear Media" className="absolute top-4 right-4 p-2 bg-white rounded-full shadow text-zinc-500 hover:text-red-500"><X size={20} /></button>}
                        <input type="file" ref={fileInputRef} className="hidden" accept="image/*,video/*" onChange={handleFileUpload} aria-label="Upload File" />
                    </div>
